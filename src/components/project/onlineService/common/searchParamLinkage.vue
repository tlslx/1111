    ...
    <el-form :inline="true" :model="formInline" class="form-inline">
      <el-form-item v-for="(item, index) in data" :key="index" :label="item.label" :label-width="item.width">
        ...
        <!-- <span v-if="item.type === 'sort'">
          <i class="el-icon-sort-up custom-sort-btn" @click="sortUp"></i>
          <i class="el-icon-sort-down custom-sort-btn" @click="sortDown"></i>
        </span> -->
      </el-form-item>
      <el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 升序
    // sortUp() {
    //   this.$emit("sort-by-up");
    // },
    // // 降序
    // sortDown() {
    //   this.$emit("sort-by-down");
    // }
  }
};
</script>
