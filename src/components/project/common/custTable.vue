    ...
    <el-table
      ...
      <template v-for="(colItem, key) in cols">
        ...
        <template v-else-if="colItem.type === 'img'">
         ...
         <template v-else-if="colItem.type === 'imgfile'">
          <el-table-column
            :key="key"
            :label="colItem.label"
            :width="colItem.width"
            ...
            <template slot-scope="scope">
              <template v-if="scope.row.isimg">
                <img  :src="scope.row[colItem.prop]" style="width:100%; height: 100%;">
              </template>
              <template v-else>
          ...
          <el-table-column :key="key"
            ...
            <template slot-scope="scope">
              ...
              <template v-else v-for="(btnItem, index) in colItem.btns">
                ...
                <el-button v-else-if="btnItem.type === 'underlineText'"
                  :key="index"
                  :style="btnItem.style"
                  type="text"
                  :disabled="scope.row.isInSetting == '0'"
...
export default {
      ...
      handler: function(newV, oldV) {
        ...
        if (flag) {
          for (let item of this.data.data) {
            if (this.checkProp === item[this.propName]) {
              // console.log(2222);
  ...
  methods: {
    ...
    selectionRow(index) {
      this.$nextTick(() => {
        this.$refs.table.toggleRowSelection(this.data.data[index]);   //默认选中
      });
    }
  }
};
