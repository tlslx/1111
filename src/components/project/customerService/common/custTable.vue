          ...
          <el-table-column :key="key" :label="colItem.label" :width="colItem.width" :fixed="colItem.fixed" align="center">
            <template slot-scope="scope">
              ...
              <template v-if="colItem.changeByCondition">
                <span v-for="btnValue in colItem.btns" :key="btnValue.type" v-if="scope.row[btnValue.type]" :class="scope.row.class" v-bind:data-src="scope[colItem.prop]" @click="dealWithDiffEvent(btnValue.type, scope, colItem.prop)">
              ...
              <template v-else v-for="(btnItem, index) in colItem.btns">
                ...
                <span v-else-if="btnItem.type === 'underlineText'" :key="index" :style="btnItem.style" @click="dealWithOperate(scope, btnItem.prop, btnItem.label)">
                  {{btnItem.label}}
                </span>
