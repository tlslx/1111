<template>
  <el-page class="seat">
    <template slot="body">
      <el-header>
        <headline :label="languageSource.LocationLang.foreignPartReview"></headline>
      </el-header>
      <kindo-box>
      ...
      <kindo-box>
        <kindo-table ref="table" :url="config.api.foreignGetList" :queryParam="form" @selection-change="handleSelectionChange">
          ...
          <el-table-column :label="languageSource.CommonLang.operation" align="center" fixed="right" min-width="130">
            <template slot-scope="scope">
              <router-link :to="{ path: '/stallplan/area?primarycolor=2b579a', query: { businessKey: scope.row.schemeId, description: scope.row.description, scopeRow: scope.row, tag: 'seatForeignReview' }}">
