<template>
  <el-page class="seat">
    <template slot="body">
      <el-header>
        <headline :label="languageSource.LocationLang.workPartReview"></headline>
      </el-header>
      <kindo-box>
      ...
      <kindo-box>
        <kindo-table
          ref="table"
          :url="config.api.workDepartmentReview"
          :queryParam="form"
        >
          ...
          <el-table-column :label="languageSource.CommonLang.operation" align="center" fixed="right" min-width="130">
            <template slot-scope="scope" >
              <router-link :to="{ path: '/stallplan/reviewArea?primarycolor=2b579a', query: { scopeRow: scope.row, tag: 'workDepartment' }}"><el-button type="text" size="mini">{{languageSource.CommonLang.view}}</el-button></router-link>
            </template>
          </el-table-column>
        </kindo-table>
      </kindo-box>
