      ...
      <el-form :model="formData" size="small" :inline="true">
        <el-form-item label="交易团">
          <el-select v-model="formData.dealClusterId" clearable style="width: 150px;">
            <el-option :label="item.deptName" :value="item.deptId" v-for="(item, index) of optionDeal" :key="index"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="申请展区">
          <el-select v-model="formData.exhibitionArea" clearable style="width: 150px;">
            <el-option :label="item.exhibitionAreaName" :value="item.exhibitionAreaCode" v-for="(item, index) of optionExhi" :key="index"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    this.getexhibitionArea().then(res => {
      this.optionExhi = JSON.parse(JSON.stringify(res));
    }).catch(e => {});
    // 交易团
    this.getdelegationDepartment().then(res => {
      this.optionDeal = JSON.parse(JSON.stringify(res));
    }).catch(e => {});
