<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="品牌展位调整安排方案公示" name="first">
    <!-- <div> -->
      <!-- 表单 -->
      ...
      <div class="paginationTop floatLeft">
        ...
        <!-- <el-button type="primary" class="formStyle" size="small" @click="addExhibition">公示</el-button> -->
      </div>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    // 下载
    handDownload() {
      let url = `${this.paginationData.currentPage}&size=${this.paginationData.total}&exhibitionArea=${this.formData.exhibitionArea}&dealClusterId=${this.formData.dealClusterId}&cantonFairCode=${this.formData.cantonFairCode}&coceralId=${this.formData.coceralId}&companyName=${this.formData.companyName}&token=${localStorage.getItem("USER_TOKEN")}`;
      window.open(process.env.API_NA_URL + "/api/SchemePublicity/reschedule/download?current=" + url, "_parent");
    },
    // 公示 change 事件
    handleChange() {
    ...
    submitPublish(fname) {
      this.$refs[fname].validate((valid) => {
        if (valid) {
          ...
          this.getlistPageReschedule(publishparm)
          .then(res => {
            let parmdat = [];
            for (let i = 0; i < res.records.length; i++) {
              if (res.records[i].publicityId == null || res.records[i].publicityId === "" || res.records[i].status === "0") {
                parmdat.push(res.records[i]);
              }
            }
            this.publicityschemePublicity(parmdat).then(resa => {
