...
export default {
  ...
  methods: {
    ...
    // 下载
    handDownload() {
      let url = `${this.paginationData.currentPage}&size=${this.paginationData.total}&exhibitionArea=${this.formData.exhibitionArea}&dealClusterId=${this.$store.getters.userInfo.org.deptId}&cantonFairCode=${this.formData.cantonFairCode}&coceralId=${this.formData.coceralId}&companyName=${this.formData.companyName}&token=${localStorage.getItem("USER_TOKEN")}`;
        ...
        if (valid) {
          ...
          .then(res => {
            ...
            for (let i = 0; i < res.records.length; i++) {
              if (res.records[i].publicityId == null || res.records[i].publicityId === "") {
