<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="品牌展位调整安排方案公布(特殊)">
...
export default {
  ...
  methods: {
    ...mapActions('brandBooth', [
      "getListpageRePublish", // 品牌展位调整安排方案公布
      "rePublishschemePublicity" // 外贸司公布调整安排方案
    ]),
    ...mapActions('nainformation', ['getexhibitionArea', 'getdelegationDepartment', 'getassociationDepartment']),
    // 初始化页面数据
    getPage() {
      this.getListpageRePublish(this.formQuery).then(res => {
    ...
    submitPublish(fname) {
      ...
      this.$refs[fname].validate((valid) => {
        if (valid) {
          ...
          this.getListpageRePublish(publishparm)
          .then(res => {
            let parmdat = [];
            for (let i = 0; i < res.records.length; i++) {
              parmdat.push(res.records[i]);
            }
            this.rePublishschemePublicity(parmdat).then(resa => {
              this.$nextTick(() => {
