        ...
        <el-card>
          ...
          <el-form :inline="true" :model="formData" class="demo-form-inline" size="small">
            <el-form-item label="设置类型">
              <el-select v-model="formData.setTheType" placeholder="请选择" clearable @change="changeSetType">
                <el-option
            ...
            <el-form-item label="设置对象">
              <el-select v-model="formData.setObject" placeholder="请选择" clearable @change="changeSetObj">
                <el-option
            ...
            <el-form-item label="使用主体">
              <el-select v-model="formData.useTheme" clearable placeholder="请选择">
                <el-option
          ...
          <el-table
            ...
            align="center"
            border>
            <el-table-column align="center" min-width="120" prop="stageName" label="阶段"></el-table-column>
            ...
            <el-table-column align="center" min-width="120" prop="useScope" label="设置对象">
              <template slot-scope="scope">
            ...
            <el-table-column align="center" min-width="120" prop="useScopeObjectId" label="使用主体">
              <template slot-scope="scope">
          ...
          <!-- 新增弹窗 -->
          <el-dialog title="新增" :visible.sync="isShow" v-if="isShow" :close-on-click-modal="false" width="600px" @close="closeDialog">
            <el-card>
              <div>
                <el-form :model="formDataTwo" :rules="rules" size="small" ref="ruleForm">
                  ...
                  <el-form-item label="阶段" prop="sectionType">
                    <el-select
                  ...
                  <el-form-item label="设置对象" prop="setObject">
                    <el-select v-model="formDataTwo.setObject" clearable placeholder="请选择" style="width: 280px;" @change="changeObjAdd">
                      <el-option
                  ...
                  <el-form-item label="使用主体" prop="useTheme">
                    <el-select v-model="formDataTwo.useTheme" clearable placeholder="请选择" style="width: 280px;">
                      <el-option
                  ...
                  <el-form-item>
                    <el-button type="danger" @click="addServe('ruleForm')">保存</el-button>
                    <el-button @click="handAddCancel">取消</el-button>
                  </el-form-item>
                </el-form>
              </div>
            </el-card>
          ...
          <el-dialog title="修改" :visible="isEditBox" width="500px" :close-on-click-modal="false" @close="closeEditBox">
            <el-card>
              <el-form :model="formDataEdit">
                <el-form-item label="设置类型">
                  <el-select style="width: 280px;" clearable disabled v-model="formDataEdit.stageType">
                    <el-option
                      disabled
                      v-for="item in optionsSet"
                      :key="item.value"
                ...
                <el-form-item label="子项">
                  <el-select style="width: 280px;" clearable disabled v-model="formDataEdit.itemCode" @change="addChildren">
                    <el-option
                ...
                <el-form-item label="设置对象">
                    <el-select disabled v-model="formDataEdit.useScope" clearable placeholder="请选择" style="width: 280px;" @change="changeObjAdd">
                      <el-option
                        v-for="item in optionsObj"
                        :key="item.value"
                        :label="item.label"
                  ...
                  <el-form-item label="使用主体">
                    <el-select disabled clearable v-model="formDataEdit.useScopeObjectId" placeholder="请选择" style="width: 280px;">
                      <el-option
                        v-for="item in optionsUseAdd"
                        :key="item.deptName"
                        :label="item.deptName"
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        sectionType: [{ required: true, message: "该字段为必填项", trigger: "blur" }],
        setObject: [{ required: true, message: "该字段为必填项", trigger: "blur" }],
        useTheme: [{ required: true, message: "该字段为必填项", trigger: "blur" }],
  ...
  created() {
    ...
    this.getcompany().then(res => {
      let req = JSON.parse(JSON.stringify(res));
      for (let i = 0; i < req.length; i++) {
        let resp = {};
        resp["deptName"] = req[i].companyName;
        resp["deptCode"] = req[i].companyId;
        this.company.push(resp);
      }
    }).catch(e => {});
  ...
  mounted() {
    // this.optionsUse = this.localstora.delegation.concat(this.localstora.businessAssociation).concat(this.company);
  },
  // 设置对象过滤器
  filters: {
    statusSetObj(val) {
  ...
  methods: {
    ...
    // 查询设置对象change改变事件
    changeSetObj() {
      // this.optionsUse = this.localstora.delegation.concat(this.localstora.businessAssociation)
      // .concat(this.company);
      this.optionsUse = [];
      ...
      if (this.formData.setObject === "1") {
        ...
        this.optionsUse = this.localstora.delegation;
      } else if (this.formData.setObject === "3") {
        this.optionsUse = this.localstora.businessAssociation;
      } else if (this.formData.setObject === "4") {
        this.optionsUse = this.company;
      }
    },
    // 新增设置对象change改变事件
    changeObjAdd() {
      this.optionsUseAdd = [];
      this.formDataEdit.useScopeObjectId = "";
      ...
      if (this.formDataTwo.setObject === "1") {
        ...
        this.optionsUseAdd = this.localstora.delegation;
      } else if (this.formDataTwo.setObject === "3") {
        this.optionsUseAdd = this.localstora.businessAssociation;
      } else if (this.formDataTwo.setObject === "4") {
        this.optionsUseAdd = this.company;
      }
    },
    // 新增设置类型改变时触发
    changeSet() {
      ...
      this.$refs[fname].validate((valid) => {
        if (valid) {
          ...
          if (this.formDataTwo.data[0].childOptions === "") {
            ...
            this.addDataSubmit.scheduleSetting.deadline = $date.format(this.formDataTwo.data[0].dateOne, 'MM-dd');
            ...
            if (this.formDataTwo.data.length === 1) {
              ...
              for (let k = 0; k < this.formDataTwo.data.length; k++) {
                let caSubmit = {
                  itemCode: this.formDataTwo.data[k].childOptions,
                  deadline: $date.format(
                    this.formDataTwo.data[k].dateOne,
                    "MM-dd"
                  ),
              ...
              this.childAddSubmit.scheduleSetting["remark"] = this.formDataTwo.remark;
    ...
    // 下载  specialSchedule: "1"
    onSubmit() {
      let url = "stageCode=" + this.obj.stageCode + "&specialSchedule=1" + "&deadline=" + this.obj.deadline + "&stageType=" + this.obj.stageType + "&useScope=" + this.obj.useScope + "&useScopeObjectId=" + this.obj.useScopeObjectId;
