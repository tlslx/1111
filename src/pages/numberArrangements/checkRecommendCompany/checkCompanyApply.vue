<template>
  <el-tabs v-model="activeName" :inline="true">
    <el-tab-pane label="审核申请企业信息" name="first">
      ...
      <el-table :data="confirmCompanyInfoData.records" ref="multipleTable" v-loading="loading" @select="handleSelectionChange" @select-all="handleSelectionChange" @selection-change="handleCheckedChange" :row-key="getRowKey" border>
        ...
        <el-table-column align="center" label="企业名称" prop="companyName" width="120">
          <template slot-scope="scope">
            <a href="javacript:;" @click="goConCompanyApt(scope.row.brandApplyId, 'info')">{{ scope.row.companyName }}</a>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作" width="200" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" @click="handleTablePass(scope.row, '1', '')">通过</el-button>
            <el-button type="text" @click="handleTablePass(scope.row, '2', '')">不通过</el-button>
            <el-button type="text" @click="goConCompanyApt(scope.row.brandApplyId, 'aptitude')">审核资质</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-button class="utils-button" type="primary" size="medium">下载模板</el-button>
      ...
      <!-- 不通过弹窗 -->
      <el-dialog title="不通过原因" :visible.sync="passDialog" v-if="passDialog" width="550px">
...
export default {
  ...
  methods: {
    ...
    // 前往查看企业信息页
    goConCompanyApt(id, type) {
      this.$router.push({ path: '/reviewEnterpriseQualiy', query: { id: `${id}&${type}` }});
    },
    // 点击表格里的通过or不通过
