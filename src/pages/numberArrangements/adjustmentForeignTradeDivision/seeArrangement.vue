    ...
    <el-tab-pane label="外贸司批复经外贸中心复核的展位数量调整安排方案" name="first">
      ...
      <el-table :data="rescheduleList.records" v-loading="loading" border>
        ...
        <el-table-column label="批复" align="center">
          <template slot-scope="scope">
            <el-button type="text" @click="handlePass(scope.row, '1', '')">通过</el-button>
            <el-button type="text" @click="handlePass(scope.row, '2', '')">不通过</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  created() {
    this.loading = true;
    this.getRescheduleListQueryPage().then(res => {
