    ...
    <el-tab-pane label="未安排企业评分比对表" name="first">
      <el-form :model="queryForm" :inline="true" label-width="">
        ...
        <el-form-item label="是否达标">
          <el-select v-model="queryForm.isStandard" clearable>
            <el-option v-for="(item, index) in isBeyondOpts" :key="index" :value="item.value" :label="item.label"></el-option>
          </el-select>
        </el-form-item>
      ...
      <el-table :data="scoreComparData.records" v-loading="loading" :default-sort = "{prop: 'regin', order: 'null'}" border>
        <el-table-column label="企业基本信息" align="center">
          ...
          <el-table-column :label="`${numDat.lastReviewExhibitionSession}届品牌展位数`" align="center">
          ...
          <el-table-column :label="`${numDat.lastExhibitionSession}届品牌展位数`" align="center">
            <template slot-scope="scope">
...
export default {
  ...
  created() {
    ...
    this.getexhibitionSession().then(res => {
      this.numDat = JSON.parse(JSON.stringify(res));
    });
    this.createdQuery();
  },
  methods: {
    ...mapActions('workDepartReturn', ['getScoreComparisonInfo',
      "getexhibitionSession" // 返回前台页面需要的展届数
    ]),
    ...mapActions('nainformation', ['getexhibitionArea', 'getassociationDepartment', 'getdelegationDepartment']),
    // 查询
    handleQuery() {
    ...
    //下载
    handleDownload() {
      let url = `isBeyond=${this.queryForm.isBeyond}
      &exhibitionArea=${this.queryForm.exhibitionArea}
      &coceralId=${this.queryForm.coceralId}
      &dealClusterId=${this.queryForm.dealClusterId}
      &isStandard=${this.queryForm.isStandard}
      ...
      window.open(`${process.env.API_NA_URL}/api/backArrange/exportScoreComparisonInfo?${url}&token=${localStorage.getItem("USER_TOKEN")}`, '_blank');
    },
    // 操作页面
