<template>
  <el-page class="seat">
    <template slot="header">
      <kindo-box :title="languageSource.AdjustmentApply.patchReleaseTitle">
        <el-form :inline="true" :model="searchForm" ref="searchForm" label-position="left" label-width="85px"
          ...
          <el-form-item prop="schemeName" :label="languageSource.LocationLang.programNameTable">
            <el-input v-model="searchForm.schemeName" clearable  class="common-style"
              :placeholder="languageSource.LocationLang.programNameTip" :maxlength="255"></el-input>
          </el-form-item>
      ...
      <kindo-box>
        ...
        >
          ...
          <el-table-column :label="languageSource.CommonLang.operation" align="center" fixed="right" min-width="130">
            <template slot-scope="scope" >
              <el-button type="text" @click="gotoPage(scope.row)" size="mini">{{languageSource.CommonLang.view}}</el-button>
            </template>
          </el-table-column>

          <div slot="control">
            <el-button type="primary" v-if="right('downloadPDF')" @click="downloadPDF" :disabled="!btnState">{{languageSource.LocationLang.downloadPDF}}</el-button>
            <el-button type="primary" v-if="right('publicity')" @click="$refs.publicityDialog.dialogVisible = true" :disabled="!publicityState" >{{languageSource.LocationLang.publicity}}</el-button>
            <el-button type="primary" v-if="right('publish')" @click="$refs.publishDialog.dialogVisible = true" :disabled="!publishState" >{{languageSource.LocationLang.publish}}</el-button>
          </div>
        </kindo-table>
      </kindo-box>
...
export default {
  ...
  methods: {
    ...
    downloadPDF () {
      ...
      ids = selectIds.join(',');
      // kindo.util.downloadFile(this.config.api.downloadPDF, { schemeId: ids });
      window.location.href = this.config.api.downloadPDF + `?schemeId=${ids}`;
    },
    gotoPage (row) {
      this.$router.push({ path: '/stallplanDemo/patchSchemeArea?primarycolor=2b579a', query: { scopeRow: row, tag: 'patchSchemePublicity', right: 'patchSchemeView', pluginBtnState: false, addState: false, notValidateArray: ['checkProgressRule', 'isRepeatArranged'] }});
    }
  }
};
</script>
