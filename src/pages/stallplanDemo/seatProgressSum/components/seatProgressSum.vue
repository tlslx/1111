<template>
  <el-page class="seat">
    <template slot="header">
      <rule-tabs
        :title="languageSource.LocationLang.progressSum"
        :languageSource="languageSource"
        activeName="progressSum"
    ...
    <template slot="body">
      <kindo-box>
        <kindo-table
          ref="table"
          :url="config.api.getScheduleData"
          :queryParam="form"
        >
          ...
          <el-table-column prop="list[0].schemeName" :label="languageSource.LocationLang.programNameTable" min-width="100" header-align="center" align="center"></el-table-column>
          ...
          <el-table-column prop="brand" :label="languageSource.LocationLang.brand" min-width="160" header-align="center" align="center">
            <template slot-scope="scope">
              {{findProgress(scope.row.list, '品牌', '特装')}}
            </template>
          </el-table-column>
          <el-table-column prop="generalSpecial" :label="languageSource.LocationLang.generalSpecial" min-width="160" header-align="center" align="center">
            <template slot-scope="scope">
              {{findProgress(scope.row.list, '一般性', '特装')}}
            </template>
          </el-table-column>
          <el-table-column prop="generalStandard" :label="languageSource.LocationLang.generalStandard" min-width="160" header-align="center" align="center">
            <template slot-scope="scope">
              {{findProgress(scope.row.list, '一般性', '标摊')}}
            </template>
          </el-table-column>
        </kindo-table>
      </kindo-box>
...
export default {
  name: "seatProgressSum",
  mixins: [listPageBase],
  components: {
    'ruleTabs': () => import('./ruleTabs.vue')
    // 'seatNumSum': () => import('../seatNumSum/index.vue')
  },
  data() {
    return {
      'pageId': 'seatProgressSum',
      //接口地址
      config,
      'searchForm': {
        exhibitionPeriod: '',
  ...
  methods: {
    ...
    //查找安排进度
    findProgress (rows, exhibitionProperty, exhibitionKind) {
      ...
      let showText = currentRow[0] ? currentRow[0].progressStatus : '';
      return showText;
    },
    //tab点击
    handleClick(tab) {
