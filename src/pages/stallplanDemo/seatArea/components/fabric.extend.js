...
const [OFFSET_WIDTH, OFFSET_HEIGHT] = [80, 60];
//展示模式
...
const getExhibitionDrawData = (exhibitionLines, xScale, yScale, startX, startY) => {
  ...
  exhibitionLines.forEach(item => {
    ...
    if (item.startPointX === item.endPointX) { // 左右
      ...
      item.startPointY === yMin ? (t = direction) : (b = direction);
    }
  });
  return {
    startPointX: (xMin - startX) * xScale + OFFSET_WIDTH,
    startPointY: (yMin - startY) * yScale + OFFSET_HEIGHT,
...
const getExhibitionDesc = (planInfo) => {
  ...
  let desc = `${enterprise.enterpriseAbbreviation}(${planInfo.num})`;
...
const drawText4SingleExhibition = (ctx, exhibition, openDirection) => {
  ...
  const textArray = branchDescribtion(ctx, text, openWidth);

  textArray.forEach((item, index) => {
    const offset = (textArray.length - index - 1) * (fontSize + 2);
    const yCoord = openDirection === 'b' ? y + offset : y - offset;
...
const drawText4RelationExhibition = (ctx, exhibition) => {
  ...
  if (isNeedDesc) {
    ...
    textArray.forEach((item, index) => {
      const offset = (textArray.length - index - 1) * (fontSize + 2);
      const yCoord = exhibition.planInfo.openDirection === 'b' ? y + offset : y - offset;
      ctx.fillText(item, 0, yCoord);
      exhibition._textTrack({
        'style': `font: ${ctx.font}; opacity: 1;`,
        'x': -ctx.measureText(item).width / 2,
...
const Exhibition = fabric.util.createClass(fabric.Rect, {
  ...
  initialize: function(exhibition) {
    ...
    const defaultOption = {
      fill: notDisabled ? 'white' : COLOR_DISABLED,
      stroke: 'black',
      strokeDashArray: [2, 4],
      hoverCursor: notDisabled ? 'pointer' : 'not-allowed',
      width: exhibition.width,
      height: exhibition.height,
      left: exhibition.startPointX,
      top: exhibition.startPointY,
      selectable: notDisabled
    };
...
const IslandExhibition = fabric.util.createClass(Exhibition, {
  ...
  _render: function (ctx) {
    ...
    this._pathTrack(ctx, `M ${-wPercent} ${-y} L ${-wPercent} ${y}
                    ...
                    L${x + markLineLength} ${-hPersent - markLineLength}`);
    /*ctx.fillStyle = 'black';
    ctx.font = ctx.font.replace('10', '8');
    ctx.fillText('24000', 0, -this.height * 0.4);
    this._textTrack({
      'style': `font: ${ctx.font}; opacity: 1;`,
      'x': -ctx.measureText('24000').width / 2,
      'y': -this.height * 0.4,
      'text': '24000'
    });
    ctx.rotate(-90 * Math.PI / 180);
    ctx.fillText('20250', 0, -this.width * 0.4);
    this._textTrack({
      'style': `font: ${ctx.font}; opacity: 1;`,
      'x': -ctx.measureText('20250').width / 2,
      'y': -this.width * 0.4,
      'text': '20250',
...
fabric.Canvas.prototype.addStand = function(data, xScale, yScale, startX, startY) {
  ...
  const extendData = Object.assign(data, drawData);
  if (data.isIsland) {
...
// 添加基础设施设备和辅助区域
fabric.Canvas.prototype._addFacility = function (facility, imgData, startX, startY, xScale, yScale) {
  ...
  const imgInstance = new fabric.Image(icon, {
    left: (facility.pointX - startX - (isInfrastructure ? facility.length / 2 : 0)) * xScale + OFFSET_WIDTH,
    top: (facility.pointY - startY - (isInfrastructure ? facility.width / 2 : 0)) * yScale + OFFSET_HEIGHT,
    scaleX: (facility.length * xScale) / icon.naturalWidth,
...
fabric.Canvas.prototype.drawGraph = function(data, state) {
  ...
  boothInfo.channels.forEach(item => {
    ...
    const channelText = new fabric.Text(item.channelCode, {
      top: channel.startPointY,
      left: channel.startPointX + 2,
      selectable: false,
  ...
  // 绘制标题
  const [mainSize] = [20, 14];
  const mainTitle = new fabric.Text(schemaInfo.exhibitionAreaName + ",展厅编号: " + boothInfo.exhibitionHall.exhibitionHallCode, {
    top: OFFSET_HEIGHT - mainSize - 4,
    left: OFFSET_WIDTH,
    fontSize: mainSize,
    selectable: false
  });
  /*const subTitle = new fabric.Text("(展位数328个,可搭展展位数534个)", {
    top: startY - subSize - 4,
    left: startX + mainTitle.width + 20,
    fontSize: subSize,
    selectable: false
  });*/
  ...
  boothInfo.facilities.forEach(facility => {
    if (facilityIcons[facility.facilityIcon]) {
      this._addFacility(facility, facilityIcons[facility.facilityIcon], startX, startY, xScale, yScale);
    } else {
      this.getIcon(facility.facilityIcon)
      .then(res => {
        facilityIcons[facility.facilityIcon] = res;
        this._addFacility(facility, res, startX, startY, xScale, yScale);
      });
    }
  });
...
fabric.Canvas.prototype.invertIslandExhibitions = function (islandExhibitions) {
  islandExhibitions.forEach(islandExhibition => {
    ...
    const enterprise = {
      ...
      mainProductKeywordCh: maxEnterprise.mainProductKeywordCh,
      exhibitTypeTwo: maxEnterprise.exhibitTypeTwo,
...
fabric.Canvas.prototype.planIslandExhibition = function(enterprises, checkedList) {
  ...
  checkedList.forEach(islandExhibition => {
    islandExhibitions = islandExhibitions.concat(islandExhibition.split);
  });
  enterprises.forEach(enterprise => {
    const recommendNum = enterprise.boothNum; //企业推荐数
...
fabric.Canvas.prototype.addLegend = function(legends) {
  ...
  legends.forEach(item => {
    const legend = new Legend(item.color, item.text, this.width / 2.5 + lastTextWidth, 20);
    lastTextWidth += LEGEND_WIDTH + item.text.length * 12 + 6;
    this.add(legend);
  });
  this.renderAll();
};
    ...
    exhibitions.filter((object) => object.planInfo).forEach((object) => {
      if (object.planInfo.enterprise.companyId === item.companyId) {
        ...
        object.set('contrastSchema', {
          ...
          "mainProductKeywordCh": item.mainProductKeywordCh
        });
      }
...
fabric.Canvas.prototype.onResetQuery = function() {
  ...
  .forEach((object) => {
    if (object.planInfo) {
      ...
      object.set('fill', enterprise.exhibitsSpecialAreaColor || COLOR_PLANED);
    } else {
...
// 主副通道规则校验(品牌展位)
fabric.Canvas.prototype.mainChannelValidate = function () {
  // 获取当前展区安排的企业
  const planedExhibitions = this._objects.filter(object => object.selectable && object.planInfo);
  if (this.showChannelvalidate) {
    this.showChannelvalidate = false;
  ...
  const dealChannelValidate = (exhibition) => {
    ...
    if (higher.length) {
      addRules(this, "mainChannel", higher.concat([enterprise]), "channel");
      higher.concat([enterprise]).forEach(item => {
...
fabric.Canvas.prototype.ruleValidate = function (enterprise, islandExhibitions = []) {
  ...
  const validateFunc = {
    ...
    checkProgressRule: function () {
      const recommendNum = enterprise.boothNum || enterprise.recommendedQuantity; // 企业推荐展位数
