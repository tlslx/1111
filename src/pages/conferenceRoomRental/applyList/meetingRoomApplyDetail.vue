    ...
    <el-tab-pane :label="label" name="first">
      <section style="padding: 10px;">
        ...
        <div v-for="(item, index) in resData.meetingContentListStr" :key="index">
          ...
          <el-row>
            <el-col :span="24" v-if="item.meetingPayTypeId">
              <span class="label-l">审批结果：</span>
              <span class="label-r">
                <!--meetingPayTypeId null  不显示 123 受理；4以上不受理-->
                <span v-if="item.meetingPayTypeId==1">属商务部各司（局）、交易团、商协会组织的非商业性会议，同意举办，并免收会议室及设备使用费用。</span>
                <span v-if="item.meetingPayTypeId==2">属各交易团、商协会组织的商业性会议，同意举办，按标准收取会议室及设备使用费用。</span>
                <span v-if="item.meetingPayTypeId==3">属VIP参展企业或其他机构组织会议、论坛，同意举办，按标准收取会议室及设备使用费用。</span>
                <span v-if="item.meetingPayTypeId==4">属不适合在广交会举办的会议论坛，不承接安排。<span v-if="item.meetingFaillReason">原因：{{item.meetingFaillReason}}</span></span>
              </span>
            </el-col>
          </el-row>
        </div>
    ...
    methods: {
      init() {
        ...
        this.$http.get(this.baseApi + '/api/customservice/meetingRent/getMeetingApply', data).then(res => {
          // this.resData = res;
          for (let name in this.resData) {
            this.resData[name] = res[name];
          }
        });
      },
      showDetail () {
