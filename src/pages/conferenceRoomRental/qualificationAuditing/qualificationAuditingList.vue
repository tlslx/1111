<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="会议室资质审核列表" name="first">
      <el-form :inline="true" :model="ruleForm" label-width="80px" class="demo-ruleForm">
        ...
        <el-form-item label="状态:" size="mini">
          <el-select v-model="ruleForm.acceptStatus" placeholder="请选择" style="width: 150px;">
            ...
            <el-option label="审核通过" value="05"></el-option>
            <el-option label="审核不通过" value="06"></el-option>
          </el-select>
        </el-form-item>
      ...
      <template>
        <el-row class="btn-row">
          ...
          <el-button type="primary" :disabled="multipleSelection.length<1" @click="submitZwk">提交展务科</el-button>
        </el-row>
        <el-table
          :data="tableData"
          border
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span v-else-if="scope.row.meetingStatus=='02'">待审核</span>
              <span v-else-if="scope.row.meetingStatus=='03'">不受理</span>
              <span v-else-if="scope.row.meetingStatus=='05'">审核通过</span>
              <span v-else-if="scope.row.meetingStatus=='06'">审核不通过</span>
          ...
          <el-table-column
            ...
            fixed="right">
            <template slot-scope="scope">
              <el-button :disabled="Number(scope.row.meetingStatus)>2" @click="handlePass(1, scope.row)" type="text" size="mini">通过</el-button>
              <el-button :disabled="Number(scope.row.meetingStatus)>2" @click="handleReject(1, scope.row)" type="text" size="mini">驳回</el-button>
              <el-button :disabled="Number(scope.row.meetingStatus)>2" @click="submitZwk(1, scope.row)" type="text" size="mini">提交展务科</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
  ...
  export default {
    ...
    methods: {
      init() {
        ...
        this.$http.get(this.baseApi + '/api/customservice/meetingRent/listApplyCondition', data).then(res => {
        ...
        if (index !== 1) {
          if (this.multipleSelection.length > 0) {
            this.multipleSelection.forEach(item => {
              ids.push({ meetingId: item.meetingId, taskId: item.taskId });
            });
          } else {
            this.$message({
              message: '请先勾选列表数据',
          ...
          ids.push({ meetingId: row.meetingId, taskId: row.taskId });
        }
        this.$confirm('确认提交到展务科?', '提示', {
      ...
      goCompanyDetail(row, index) {
        this.$router.push({
          path: '/qualificationAuditingDetail',
          query: {
      ...
      auditCondition(params) {
        this.$http.post(this.baseApi + '/api/customservice/meetingRent/auditCondition', params).then(res => {
          this.$message({
            type: 'success',
            message: '操作成功'
          });
