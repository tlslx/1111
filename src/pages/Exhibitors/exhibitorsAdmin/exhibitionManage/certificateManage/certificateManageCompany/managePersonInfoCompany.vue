...
<template>
  <div class="page-container">
    ...
    <!-- <el-tabs v-model="pageActiveName">
      <el-tab-pane label="管理人员信息(企业)" name="one"> -->
          ...
          <el-form :model="addListForm" label-width="114px">
            <el-row :getter="10">
              ...
              <el-form-item label="公司名称:" size="small" prop="name">
                <el-select style="width: 90%" v-model="addListForm.orgId">
                  <el-option v-for="item in this.allCompanyOfAgentData" :key="item.value" :value="item.value" :label="item.label"></el-option>
                </el-select>
              </el-form-item>
              </el-col>
            </el-row>
        ...
        <el-dialog title="编辑" :visible.sync="dialogEdit" width="70%" :show-close="false" @close="handleEditClose">
          <el-form label-width="114px">
            <el-row :getter="10">
              <el-col :span="12">
              <el-form-item label="公司名称:" size="small" prop="name">
                <el-select style="width: 90%" v-model="editListForm.orgId" disabled>
                  <el-option v-for="item in this.allCompanyOfAgentData" :key="item.value" :value="item.value" :label="item.label"></el-option>
                </el-select>
              </el-form-item>
              </el-col>
...
export default {
  data() {
    return {
      ...
      addListForm: {
        orgId: '', name: '', mail: '', sex: '', idType: '', idNo: '', peopleType: '', idCardExpireStartTime: '',
  ...
  computed: {
    ...mapGetters('certificateManage', ['managePeopleInfoData', 'peopleInfoDetailsData', 'peopleTypeSelectData', 'allCompanyOfAgentData']),
  ...
  methods: {
    ...mapActions('certificateManage', ['getManagePeopleInfoList', 'postUpdateOrAddPeople', 'getPeopleInfoDetails',
      'postDeletePeopleInfo', 'getPeopleTypeSelect', 'getAllCompanyOfAgent']),
    ...
    handleEditDialog(id) { // 编辑弹框
      ...
      this.getPeopleInfoDetails(this.detailsParams).then(res => {
        this.editListForm.orgId = this.peopleInfoDetailsData.orgId;
  ...
  async created() {
    ...
    this.getAllCompanyOfAgent(); // 获取企业名称
  }
};
</script>
