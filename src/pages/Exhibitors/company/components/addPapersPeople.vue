...
export default {
  ...
  computed: {
    ...
    showPeople() {
      ...
      if (obj.idScan) {
        this.sfzAddFileList = [
        { name: '证件照', url: obj.idScan ? process.env.API_POT_URL + '/api/ifoFile/getImg?fileId=' + obj.idScan : '' }];  
      } else {
        this.sfzAddFileList = [];
      }
      if (obj.cardPic) {
        this.zjAddFileList = [
        { name: '扫描件', url: obj.cardPic ? process.env.API_POT_URL + '/api/ifoFile/getImg?fileId=' + obj.cardPic : '' }];  
      } else {
        this.zjAddFileList = [];
      }
