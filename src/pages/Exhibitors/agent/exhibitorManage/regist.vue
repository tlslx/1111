      ...
      <span class="remarke">*号为必填项※号为必填项</span>
        ...
        ref="registForm">
          ...
          <el-form-item label="招展代理" class="inline-item" size="small" prop="exhibitionAgentId" >
            <el-select v-model="registForm.exhibitionAgentId" placeholder="请选择" disabled>
              <el-option
...
export default {
  ...
  computed: {
    ...mapGetters('regist', ['getListAll', 'companyTypeOptionsData', 'industryTypeOptionsData', 'exhibitListAllData', 'loginInfoData']),  // 用户新增
  ...
  methods: {
    ...
    ...mapActions('regist', ['getListAllInfo', 'getConfirmIsRepeatInfo', 'getAllCountryInfo',
      'addInfo', 'getAllAgentsInfo', 'getCompanyTypeOptions', 'getIndustryTypeOptions', 'getExhibitListAll', 'getLoginInfo']),
      // getLoginInfo 招展代理下拉框
    // 查询企业名称
    research() {
      let param = {
        current: 1,
        ...
        if (res === "1") {
          this.$confirm('企业未注册，请填写注册信息', '提示', {
            ...
            this.getLoginInfo().then(() => {
              this.registForm.exhibitionAgentId = this.loginInfoData.exhibitionAgentId;
            }).then(() => {
              this.exhibitionAgents = [{ exhibitionAgentId: this.loginInfoData.exhibitionAgentId,
                agentCh: this.loginInfoData.agentCh }];
            });
            // this.getAllAgentsInfo().then(resT => { // 获取招展代理 原来的下拉框
