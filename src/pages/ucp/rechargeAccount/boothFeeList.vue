<template>
  <el-page ref="boothFeeList">
    <template slot="header">
      <!--页面对话框-->
      <el-dialog :title="(child.action==='add'? languageSource.CommonLang.add :child.action==='edit'? languageSource.CommonLang.edit : languageSource.CommonLang.view) + languageSource.CommonPayLang.boothFee" :close-on-click-modal="false" top="15px" :visible.sync="childVisible" width="90%">
        <kindo-box>
      ...
      <kindo-box :title="languageSource.CommonPayLang.boothFeeRegList" icon="fa-search">
        <el-form :model="forms" :rules="formsRules" label-position="right" inline @submit.native.prevent @keyup.enter.native="onQuery">
          ...
          <el-form-item>
            <el-button icon="el-icon-search" type="primary" @click="onQuery" v-if="right('boothFeeListQuery')">{{ languageSource.CommonLang.query }}</el-button>
            <el-button type="info" @click="onResetQuery" v-if="right('boothFeeListReset')">{{ languageSource.CommonLang.reset }}</el-button>
          </el-form-item>
        </el-form>
          ...
          <kindo-table ref="table" :url="api.get" :queryParam="form">
            ...
            <el-table-column :label="languageSource.CommonLang.operation" width="150px" align="center" fixed='right'>
              <template slot-scope="scope">
                <el-button type="text" @click="editList(scope.row,'id', 'child', api.getbyid)" v-if="right('boothFeeListEdit')">{{ languageSource.CommonLang.edit }}</el-button>
                <el-button type="text" @click="deleteData(scope.row)" v-if="right('boothFeeListDelete')">{{ languageSource.CommonLang.delete }}</el-button>
              </template>
            </el-table-column>
          </kindo-table>
        </kindo-box>
  ...
  data() {
    return {
      ...
      api: {
        ...
        get: kindo.config.api.unifiedCollectUrl + "/api/recharge/prereceiveRecords/getListPage",
        delete: kindo.config.api.unifiedCollectUrl + "/api/recharge/prereceiveRecords/deleteById",
        update: kindo.config.api.unifiedCollectUrl + "/api/recharge/prereceiveRecords/saveOrUpdate"
      },
      forms: {
        unitName: "",
        billNo: "",
