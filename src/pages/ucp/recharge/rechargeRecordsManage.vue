    ...
    <template slot="header">
      ...
      <kindo-box :title="languageSource.CommonPayLang.prepayChargeRecords" icon="fa-search">
        <el-form :model="forms" class="query_form" :rules="formsRules" ref="forms" label-position="right" label-width="80px" inline @submit.native.prevent @keyup.enter.native="onQuery">
          ...
          <el-form-item>
            <el-button icon="el-icon-search" type="primary" v-if="right('innerRechargeRecordsQuery')" @click="onQuery">{{ languageSource.CommonLang.query }}</el-button>
            <el-button type="info" v-if="right('innerRechargeRecordsReset')" @click="onResetQuery">{{ languageSource.CommonLang.reset }}</el-button>
          </el-form-item>
        </el-form>
      ...
      <el-page-data>
        <kindo-box>
          <kindo-table ref="table" :url="api.get" :queryParam="form">
            ...
            <el-table-column prop="rechargeAccountNum" :label="languageSource.CommonPayLang.prepayAccount" width="120" show-overflow-tooltip header-align="center"></el-table-column>
            <el-table-column prop="companyName" :label="languageSource.CommonPayLang.companyName" width="200" show-overflow-tooltip header-align="center"></el-table-column>
            ...
            <el-table-column :label="languageSource.CommonLang.operation" width="180" align="center" fixed="right">
              <template slot-scope="scope">
                <el-button type="text" v-if="right('innerRechargeRecordsView')" @click="viewList(scope.row,'id', 'child', api.getbyid)">{{ languageSource.CommonLang.view }}</el-button>
                <el-button type="text" v-if="right('innerRechargeRecordsViewReceipt')" @click="viewList(scope.row,'billNo', 'bankReceipt', api.getBankReceiptByBillNo)">{{ languageSource.CommonPayLang.bankReceipt }}</el-button>
              </template>
            </el-table-column>
          </kindo-table>
        </kindo-box>
...
export default {
  name: "innerRechargeRecords",
  mixins: [tableMixIn, listPageBase],
  data() {
    return {
      pageId: "innerRechargeRecords",
      languageSource: {
        CommonPayLang: {},
        CommonLang: {}
      },
      api: {
        get: kindo.config.api.unifiedCollectUrl + "/api/recharge/rechargeRecords/getListPage",
        getbyid: kindo.config.api.unifiedCollectUrl + "/api/recharge/rechargeRecords/getSingleById",
        getBankReceiptByBillNo: kindo.config.api.unifiedCollectUrl + "/api/recharge/rechargeRecords/getBankReceiptByBillNo"
      },
      forms: {
        billNo: "",
        companyName: "",
