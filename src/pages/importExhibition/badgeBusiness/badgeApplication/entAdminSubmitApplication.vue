      ...
      <kindo-box :title="source.language.entAdminSubmitApplication">
        ...
        <el-form inline ref="applicationForms" :rules="applicationRules" :model="applicationForms" @submit.native.prevent @keyup.enter.native="onQuery" label-position="right" v-if="isOnQuery">
          <el-form-item :label="source.language.nameTable" label-width="60px" prop="name">
            <el-input @blur="(ev)=>ev.target.value = ev.target.value.trim()" v-model="applicationForms.name" :placeholder="source.language.pleaseEnter" class="inputclass"></el-input>
          </el-form-item>
          <el-form-item :label="source.language.identityNumber" label-width="90px" prop="idCardNo">
            <el-input @blur="(ev)=>ev.target.value = ev.target.value.trim()" v-model="applicationForms.idCardNo" :placeholder="source.language.请输pleaseEnter入" class="inputclass"></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
          <el-tab-pane :label="source.language.badgeApplication" name="certificationApplication">
            <kindo-box>
              <el-row>
                <el-col>
                  <div class="table-title">{{source.language.indexTipOneA}} {{promptMessage.exhibitionPeriod}} {{source.language.indexTipOneB}} {{promptMessage.levelName}} {{source.language.indexTipOneC}} {{promptMessage.totalQuotaNumber}} {{source.language.indexTipOneD}} {{promptMessage.levelName}} {{source.language.indexTipOneC}} {{promptMessage.remainQuotaNumber}} {{source.language.indexTipOneE}}</div>
                </el-col>
              </el-row>
              <kindo-table ref="certificationApplicationTable" :url="api.getApplication" :queryParam="applicationForm" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="mail" :label="source.language.mailbox" show-overflow-tooltip min-width="120"></el-table-column>
              ...
              <el-form :model="submitApplicationForm" inline ref="submitApplicationForm" :rules="submitApplicationFormRules" label-position="right" class="footerclass">
                ...
                <el-form-item :label="source.language.startingTime" label-width="80px" prop="effectStartDate">
                  <el-date-picker :picker-options="exhibitionPeriodTimeOption" value-format="yyyy-MM-dd" :title="kindo.util.formatDate(submitApplicationForm.effectStartDate)" v-model="submitApplicationForm.effectStartDate" type="date" format="yyyy 年 MM 月 dd 日" :placeholder="source.language.selectDate" class="inputclass">
                  </el-date-picker>
                </el-form-item>
              ...
              <el-row type="flex" justify="center">
                <el-col :span="6">
                  <el-button v-if="right('add')" type="primary" @click="addPersonalUser">{{source.language.addPersonalUser}}</el-button>
...
export default {
  name: "entAdminSubmitApplication",
  mixins: [listPageBase],
  data() {
    var _this = this;
    return {
      // pageId: "entAdminSubmitApplication",
      api: {
        getApplication: kindo.config.api.importExbUrl + "/api/BmMcApply/getPersonUserList",
        getHint: kindo.config.api.importExbUrl + "/api/BmMcApply/enterpriseQuotaHint", //查办证申请表的提示信息
        ...
        getAlreadyApplied: kindo.config.api.importExbUrl + "/api/BmMcApply/getIsBadgeList",
        entSubmitApplication: kindo.config.api.importExbUrl + "/api/BmMcApply/enterpriseQuotaApply",
  ...
  methods: {
    ...
    //期数过滤显示
    periodFormat(row) {
      let periodLabel = row.exhibitionPeriod ? row.exhibitionPeriod : row.period;
    ...
    levelNameFormat(row) {
      ...
      for (let i = 0; i < this.source.feeTypeQueryOptions.length; i++) {
        if (this.source.feeTypeQueryOptions[i].value === levelLabel) {
          levelLabel = this.source.feeTypeQueryOptions[i].label;
        }
      }
      return levelLabel;
    },
    ...
    getCurrentUserInfo() {
      ...
      this.$http.get(this.api.getUserInfo, params).then(res => {
        ...
        this.applicationForms.enterpriseId = this.currentUserInfo.enterpriseId;
        this.alreadyAppliedForms.enterpriseId = this.currentUserInfo.enterpriseId;
        this.alreadyAppliedForms.enterpriceCode = this.currentUserInfo.enterpriseNo;
    ...
    //选择期数档位后，提示当前企业剩余指标信息
    getPromptMessage() {
      let params = {
        json: {
          customQueryParams: {
            enterpriseId: this.currentUserInfo.enterpriseId,
            period: this.submitApplicationForm.exhibitionPeriod ? this.submitApplicationForm.exhibitionPeriod : "1",
            levelName: this.submitApplicationForm.levelName ? this.submitApplicationForm.levelName : "1"
          },
          page: {
            current: 1,
            size: 10
      ...
      this.$http.get(this.api.getHint, params).then(res => {
        this.promptMessage = res.records[0] ? res.records[0] : {};
        //期数，档位返回的是数字，需转换后显示
        this.promptMessage.exhibitionPeriod = this.periodFormat(this.promptMessage);
        this.promptMessage.levelName = this.levelNameFormat(this.promptMessage);
      });
    },
    //提交申请
    submitApplication() {
      ...
      this.$refs.submitApplicationForm.validate(valid => {
        if (valid) {
          if (this.submitApplicationSelection.length > this.promptMessage.remainQuotaNumber) {
            this.$message({
              message: this.source.language.submitApplicationTip,
              type: "warning"
            });
            return;
          }
          for (let i = 0; i < this.submitApplicationSelection.length; i++) {
            params.push({
              ...
              badgeTypeCode: 91,
              id: this.submitApplicationSelection[i].id,
              ...
              mail: this.submitApplicationSelection[i].mail,
          ...
          this.$http.postJson(this.api.entSubmitApplication, params).then(res => {
            this.onQuery();
            this.$message({
    ...
    onRepeal(row) {
      ...
      let promptTips = this.source.language.repealPrompt;
      if (row.status === "BU_APPROVED") {
        promptTips = this.source.language.departmentMessage;
      } else if (row.status === "BC_APPROVED") {
        promptTips = this.source.language.centerMessage;
      }
      this.$confirm(promptTips, this.source.language.repealReminder, {
