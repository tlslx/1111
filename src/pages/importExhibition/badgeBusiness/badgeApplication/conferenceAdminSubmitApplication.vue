<template>
  <el-page ref="conferenceAdminSubmitApplication">
    <template slot="header">
      <!--查看回执弹窗-->
      <receipt-dialog ref="receiptDialog" :receiptForm="viewReceiptForm" :language="source.language"></receipt-dialog>

      <kindo-box :title="source.language.conferenceAdminSubmitApplication">
        <!--按钮栏-->
            ...
            <kindo-box>
              ...
              <el-form :model="submitApplicationForm" inline ref="submitApplicationForm" :rules="submitApplicationFormRules" label-position="right" class="footerclass">
                ...
                <el-form-item :label="source.language.startingTime" label-width="80px" prop="effectStartDate">
                  <el-date-picker :picker-options="exhibitionPeriodTimeOption" value-format="yyyy-MM-dd" v-model="submitApplicationForm.effectStartDate" type="date" format="yyyy 年 MM 月 dd 日" :placeholder="source.language.selectDate" class="inputclass">
                  </el-date-picker>
                </el-form-item>
...
export default {
  name: "conferenceAdminSubmitApplication",
  mixins: [listPageBase],
  data() {
    var _this = this;
    return {
      pageId: "conferenceAdminSubmitApplication",
      api: {
        getApplication: kindo.config.api.importExbUrl + "/api/BmMcApply/getAssemblyApplyList",
        getAlreadyApplied: kindo.config.api.importExbUrl + "/api/BmMcApply/getListAdminByComplete",
        getLevelName: kindo.config.api.importExbUrl + "/api/BmMcApply/getLevelName",
        getStartDate: kindo.config.api.importExbUrl + "/api/BmMcApply/getExhibitionSessionInfo", //获取当前开始日期可用范围信息
        ...
        if (valid) {
          for (let i = 0; i < this.submitApplicationSelection.length; i++) {
            params.push({
              ...
              enterpriceCode: this.submitApplicationSelection[i].enterpriseNo,
