        ...
        if (valid) {
          let param = {
            ...
            "pageStart": "1"
              ...
              // 获取企业信息
          this.getCompanyInfoOuterTwoInfo(param).then(res => {
            ...
            } else if (!res.records[0] && _this.form.userType === 2) {
              this.$message.error('对不起，信息不匹配或者系统找不到您的资料，请联系你的所属交易团！');
              return false;
            }
              //验证图像验证码
            _this.getCheckVerifyCodeAccountImg(_this.testCheckCodeParams).then(resT => {
              if (_this.isPhone === 1) {
                _this.isPass = 2;
        ...
        if (this.yzm === 1) {
          ...
          if (this.isPhone === 1) {
            ...
            if (this.form.userType === 2) {
              this.testCodeParams.account = this.companyId;
              this.testCodeParams.type = "3";
            } else {
