    ...
    <el-tab-pane label="审核企业资质" name="first">
      <el-collapse v-model="activeCollapse">
        <el-collapse-item title="企业基本信息：" name="0">
          ...
          <div class="newfathbtn" style="margin-top: 30px; border-top: 1px solid #ebeef5; font-weight: 900; text-align: left;">
            <div style="width: 100%; text-align: left;">是否大会禁止参展企业:</div>
          ...
          <div class="newfathbtn">
            <div class="newonebtn">企业基本信息交易团审核意见：{{companyBaseInfoData.clusterCheckStatus | checkStatus}}</div>
            <div class="newonebtn">交易团审核不通过原因：{{companyBaseInfoData.clusterCheckOpinion}}</div>
            <!--<div class="newonebtn">
        ...
        <el-collapse-item title="企业展品信息：" name="2">
          <div>
            <el-table :data="companyDetailsForm.companyProductList" v-loading="loading" max-height="390" @selection-change="rows => handleCheckedChange(rows, 'companyProduct', 'companyProductId')" border>
              ...
              <el-table-column label="交易团审核结果" align="center" prop="clusterCheckStatus">
                <template slot-scope="scope">
                  {{ scope.row.clusterCheckStatus | statusFilter }}
                </template>
              </el-table-column>
              <el-table-column label="交易团审核不通过原因" align="center" prop="clusterCheckOpinion"></el-table-column>
              <el-table-column label="审核结果" prop="workCheckStatus" align="center">
                <template slot-scope="scope">
                  {{ scope.row.workCheckStatus | statusFilter }}
                </template>
              </el-table-column>
              <el-table-column label="审核不通过原因" min-width="140px" align="center" prop="workCheckOpinion"></el-table-column>
        ...
        <el-collapse-item title="海关编码：" name="5">
          <div>
            <el-table :data="companyDetailsForm.companyCustomsCodeList" v-loading="loading" max-height="390" @selection-change="rows => handleCheckedChange(rows, 'customsCode', 'customsCodeId')" border>
              <el-table-column type="selection" width="40px"></el-table-column>
              ...
              <el-table-column label="交易团审核结果" prop="clusterCheckCustomsStatus" align="center">
                <template slot-scope="scope">
                  {{ scope.row.clusterCheckCustomsStatus | statusFilter }}
                </template>
              </el-table-column>
              <el-table-column label="交易团审核不通过原因" align="center" prop="clusterCheckCustomsOpinion"></el-table-column>
              <el-table-column label="审核结果" prop="workCheckCustomsStatus" align="center">
                <template slot-scope="scope">
                  {{ scope.row.workCheckCustomsStatus | statusFilter }}
                </template>
              </el-table-column>
              <el-table-column label="审核不通过原因" min-width="140px" align="center" prop="workCheckCustomsOpinion"></el-table-column>
              <el-table-column label="操作" align="center">
                <template slot-scope="scope">
        ...
        <el-collapse-item title="国内销售额：" name="6">
          <div>
            <el-table :data="domesticSalesList" v-loading="loading" max-height="390" border>
              ...
              <el-table-column label="经交易团审核国内销售额（万元）" align="center" prop="clusterCheckOpinion"></el-table-column>
              <el-table-column label="经工作部审核国内销售额（万元）" align="center" prop="saleCheck">
                <template slot-scope="scope">
              ...
              <!--<el-table-column label="交易团审核结果" prop="clusterCheckStatus" align="center">
                <template slot-scope="scope">
                  {{ scope.row.clusterCheckStatus | statusFilter }}
                </template>
              </el-table-column>
        ...
        <el-collapse-item title="境外注册商标：" name="8">
          <div>
            <el-table :data="tradeMarkRegistOut" v-loading="loading" max-height="390" @selection-change="rows => handleCheckedChange(rows, 'tradeMarkRegistOut', 'brandOutsideId')" border>
              ...
              <el-table-column label="交易团审核结果" align="center" prop="checkStatus">
                <template slot-scope="scope">
        ...
        <el-collapse-item title="近两年参加国际专业展：" name="13">
          <div>
            <el-table :data="assistanceProject" v-loading="loading" max-height="390" @selection-change="rows => handleCheckedChange(rows, 'actorExhibit', 'actorExhibitId')" border>
              ...
              <el-table-column label="交易团审核结果" align="center" prop="approveStatus">
                <template slot-scope="scope">
        ...
        <el-collapse-item title="近两年参与商务部对外经济合作、援助项目：" name="14">
          <div>
            <el-table :data="actorProject" v-loading="loading" max-height="390" @selection-change="rows => handleCheckedChange(rows, 'actorProject', 'actorProjectId')" border>
              ...
              <el-table-column label="交易团审核结果" align="center" prop="approveResult">
                <template slot-scope="scope">
      ...
      <el-form :model="checkForm" ref="checkForm" :rules="checkRule" label-width="140px" class="check-form">
        <el-form-item label="审核：" prop="examineStatus">
          <el-select v-model="checkForm.examineStatus" @click="handleChange">
      ...
      <div class="total-footer">
        <el-button size="medium" type="primary" @click="handleSubmit('checkForm')">提交</el-button>
        <el-button size="medium" @click="handleBanck">返回</el-button>
      </div>
    </el-tab-pane>
  </el-tabs>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters('newThemeWorkDepartCheckNumber', ['oneDemandData', 'companyBaseInfoData',
  ...
  async created() {
    this.demandRecordIdData.demandRecordId = this.$route.query.row.demandRecordId;
    ...
    if (this.$route.query.row.checkStatus !== '0') {
      this.checkForm.examineStatus = this.$route.query.row.checkStatus;
      this.checkForm.examineOpinion = this.$route.query.row.checkStatusOpinion;
    }
    this.createdQuery();
  },
  watch: {
  ...
  methods: {
    ...mapActions('newThemeCheckRecommendCompany', ['postCheckCompanyApply']),
    ...mapActions('newThemeWorkDepartCheckNumber', ['getOneDemand', 'getCompanyBaseInfo', 'postCheckCustomsCode', 'postCheckBoothUsedByWorkDepart',
    ...
    getcompanyInfo() {
      ...
      this.getCompanyBaseInfo({
        ...
        //   if (this.$route.query.row.checkStatus === '2') {
        //     this.checkForm.examineStatus = this.$route.query.row.checkStatus;
        //     this.checkForm.examineOpinion = this.$route.query.row.checkStatusOpinion;
        //   } else {
        //     this.checkForm.examineStatus = "";
        //   }
        // } else if (this.companyDetailsForm.clusterCheckStatus !== '2') {
        ...
        switch (type) {
          ...
          case 'actorProject':
            this.postCheckActorProject(submitData).then(() => {
              this.singleCreatedQuery('singleCreatedQuery', 0);
            }).catch(e => {
              this.$nextTick(() => {
                fullScreenloading.close();
              });
    ...
    handleSubmit(formName) {
      this.checkForm.ownId = this.$route.query.row.demandRecordId;
      let flag = [];
      this.noCheckList = [];
      // 企业基本信息
      flag.push(true);
      ...
      this.noCheckList.push('true');
      // 企业展品信息
      flag.push(this.handleFlagCheck(this.companyDetailsForm.companyProductList, 'workCheckStatus', '企业展品信息'));
      // 营业执照
      flag.push(this.handleFlagCheck(this.enterpriseLegal.list, 'workCheckStatus', '营业执照'));
      // 外贸经营资质证明
      flag.push(this.handleFlagCheck(this.foreignCertificate.list, 'workCheckStatus', '外贸经营资质证明'));
      // 海关编码
      flag.push(this.handleFlagCheck(this.oneDemandData.companyCustomsCodeList, 'workCheckCustomsStatus', '海关编码'));
      // 国内销售额
      // flag.push(this.handleFlagCheck(this.domesticSales.list, 'workCheckStatus', '国内销售额'));
      flag.push(true);
      this.noCheckList.push('true');
      // 产品简介
      flag.push(this.handleFlagCheck(this.productBrief.list, 'workCheckStatus', '产品简介'));
      // 境外注册商标
      flag.push(this.handleFlagCheck(this.tradeMarkRegistOut, 'workCheckStatus', '境外注册商标'));
      // 国家高新技术
      flag.push(this.handleFlagCheck(this.innovateProperty.countrySkill, 'workCheckStatus', '国家高新技术'));
      // 专利证明
      flag.push(this.handleFlagCheck(this.innovateProperty.innovateProve, 'workCheckStatus', '专利证明'));
      // 参与国家或行业标准制定
      flag.push(this.handleFlagCheck(this.propertyStandard, 'workCheckStatus', '参与国家或行业标准制定'));
      // 国际通行认证
      flag.push(this.handleFlagCheck(this.intelPassCertified, 'workCheckStatus', '国际通行认证'));
      // 近两年参加国际专业展
      flag.push(this.handleFlagCheck(this.assistanceProject, 'workCheckStatus', '近两年参加国际专业展'));
      // 近两年参与商务部对外经济合作、援助项目
      flag.push(this.handleFlagCheck(this.actorProject, 'workCheckStatus', '近两年参与商务部对外经济合作、援助项目'));
          ...
          if (valid) {
            this.postCheckCompanyApply(this.checkForm).then(() => {
              ...
              let submitObj = {
                ...
                examineUserId: this.$store.getters.userInfo.userId,
                examineUserCn: this.$store.getters.userInfo.userName,
                examineOpinion: this.domesticSalesList.length > 0 ? this.domesticSalesList[0].saleCheck : '',
                examineOrg: 'DEPARTMENT'
              };
              this.postcheckDomesticSales(submitObj).then(() => {
    ...
    handleBanck() {
      ...
      this.$router.push({ path: '/centeralCheckBoothUseTable', query: { form: queryForm, page: pageInfo }});
    },
    // 单个查询封装
        ...
        case 'domesticSales':
          this.getDomesticSales(this.demandRecordIdData).then(() => {
            ...
            for (let i = 0, l = list.length; i < l; i++) {
              list[i].edit = false;
              list[i].saleCheck = list[i].workCheckOpinion ? list[i].workCheckOpinion : list[i].clusterCheckOpinion;
            }
