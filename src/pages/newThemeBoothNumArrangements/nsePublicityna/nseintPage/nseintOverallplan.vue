  ...
  <div v-loading.fullscreen="fullLoading">
    <el-card style="padding-bottom: 24px">
      ...
      <el-form :inline="true" :model="formData" size="small">
        ...
        <!-- <el-form-item label="交易团" v-show="shadow">
      ...
      >
        ...
        <el-table-column align="center" min-width="120" :label="`第${getCes}届安排展位数`" prop="scheduleNumber"></el-table-column>
      </el-table>
      <!-- 一堆按钮 -->
      <div class="paginationTop floatLeft">
        <!-- <el-button type="danger" class="formStyle" @click="addExhibition">公示</el-button> -->
...
export default {
  ...
  methods: {
    ...mapActions("thenewThemel", [
      "getpublicityThemePublish" // 7.10 外贸司页面 整体  查询外贸司审核查看公示新题材展位安排结果后的即将公示数据
      // "addnewthemePublish" // 7.11 外贸司审定展位数量安排方案
    ]),
    ...mapActions("nacommon", ["getsearch"]),
    ...
    getListPage(Url) {
      this.tableData = [];
      this.getpublicityThemePublish(Url)
        .then(res => {
    ...
    // 下载
    handDownload() {
      let url = `${this.paginationData.currentPage}&size=${this.paginationData.total}&exhibitionArea=${this.formData.exhibitionArea}&delegationId=${this.obj.delegationId}&exhibitionType=${this.formData.exhibitionType}&productType=${this.formData.productType}&companyName=${this.formData.companyName}&token=${this.token}`;
      window.open(process.env.API_NA_URL + "/api/newthemePublish/publicity/download?current=" + url, "_parent");
    },
    // 展区事件change
    formChange() {
      this.optspe = [];
