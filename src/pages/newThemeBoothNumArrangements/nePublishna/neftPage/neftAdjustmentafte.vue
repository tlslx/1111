<template>
  <div v-loading.fullscreen="fullLoading">
    <el-card style="padding-bottom: 24px">
      ...
      <!-- 公布/公布 -->
      <el-dialog title="公布" :visible.sync="isExhibit" v-if="isExhibit" :close-on-click-modal="false" width="400px">
    ...
    submitPublish(fname) {
      this.$refs[fname].validate((valid) => {
        if (valid) {
          ...
          this.getpublishWorkScheduleNumber(publishparm)
          .then(res => {
            let parmdat = [];
            ...
            this.updatepublishWorkScheduleNumber(parmdat).then(resa => {
