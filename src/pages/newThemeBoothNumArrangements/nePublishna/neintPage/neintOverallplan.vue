...
export default {
  ...
  created() {
    ...
    // this.usInfo = this.$store.getters.userInfo; // 用户信息
  ...
  methods: {
    ...mapActions("thenewThemel", [
      "getpublishNewthemePublish", // 8.10 外贸司页面 整体  查询公布的展位數量
      "addpublishNewthemePublish" // 8.11 外贸司公布后,进行公布
    ]),
    ...
    getListPage(Url) {
      ...
      this.getpublishNewthemePublish(Url)
        .then(res => {
    ...
    handDownload() {
      ...
      window.open(process.env.API_NA_URL + "/api/newthemePublish/publish/download?current=" + url, "_parent");
    },
    // form表单的change事件
    formChange() {
      this.optspe = [];
