...
export default {
  ...
  created() {
    this.queryTradeSessionInfo(this.initParams).then(res => {
      this.loading = false;
      if (res.total > 0) {
  ...
  computed: {
    ...mapGetters('companyApplyUnionExhibition', ['queryTradeSession']),
    ...mapGetters('dictionary', ['osDictionaryData'])
  },
  methods: {
    ...mapActions('companyApplyUnionExhibition', ['queryTradeSessionInfo']),
    // 重置查询分页参数
    ...
    selectParams() {
      ...
      this.searchParamsForm.data.forEach((formVal, formIndex, formArray) => {
        if (formVal.type === 'select' && formVal.prop === 'phase') {
          this.searchParams.tradeSession.phase = formVal.value;
        }
        if (formVal.prop === 'tradeApproveResult' && formVal.type === 'select') {
          this.searchParams.tradeSession.tradeApproveResult = formVal.value;
        }
        if (formVal.type === 'select' && formVal.prop === "boothNo") {
          this.searchParams.tradeSession.boothNo = formVal.value;
        }
      });
      this.initSearchParamsPageInfo();
      this.queryTradeSessionInfo(temp).then(res => {
        _this.loading = false;
        if (res.total > 0) {
          this.pageInfo.currentPage = res.current;
