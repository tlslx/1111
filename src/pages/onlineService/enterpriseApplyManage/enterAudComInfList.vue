...
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="企业重要信息修改审核" name="0">
      ...
      <el-table border :data="tableData" :header-cell-style="tableHeaderColor" v-loading="loading">
        ...
        <el-table-column label="证明文件" width="180">
          <template slot-scope="scope">
            <el-button class="btn-text" v-if="scope.row.proofUrl" @click.native="download(scope.row)" type="text" size="small">
              下载
            </el-button>
            <el-button class="btn-text" v-if="showPreviewButton(scope.row)" @click.native="reviewOnline(scope.row)" type="text" size="small">
              在线预览
            </el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="交易团初审状态" width="200">
          <template slot-scope="scope">
            <template v-if="scope.row.approveResult === '待审核'">
        ...
        <el-table-column label="大会审核" fixed="right">
          <template slot-scope="scope">
      ...
      <el-dialog title="图片预览" :visible.sync="dialogVisible">
        ...
        <!-- <iframe :src="previewHref" width="100%" height="560" frameborder="0"></iframe> -->
      </el-dialog>
    </el-tab-pane>
  </el-tabs>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters('corpDataManagement', ['getEnterImportCompany', 'getBatchAuditImportInfo'])
  },
  created() {
    this.loading = true;
    if (this.$store.state.userInfo.businessDelegationId) {
      this.delegationId = this.$store.state.userInfo.businessDelegationId;
    }
    // 初始列表
    this.searchParams.companyImportColumnSession.deleApproveResult = 'null';
    // 获取所属交易团
    this.searchParams.companyImportColumnSession.businessDelegationId = this.delegationId;
    // this.searchParams.companyImportColumnSession.auditResults = '1';
    this.searchParamsForm.data.forEach((value) => {
      if (value.prop === 'ifdeal') {
        value.value = 'null';
      }
    });
    this.getEnterImportCompanyInfo(this.searchParams).then(res => {
      this.loading = false;
  ...
  methods: {
    ...mapActions('corpDataManagement', ['getEnterImportCompanyInfo', 'getreviewCompanyImportInfo', 'getBatchImportCompanyInfo', 'batchAuditCompanyNameAndAbbreviation']),
      ...
      if (row.proofUrl && row.proofUrl !== '') {
        ...
        if (index > -1) {
          ...
          || fileTypeName === 'JPG' || fileTypeName === 'JPEG' || fileTypeName === 'PNG' || fileTypeName === 'BMP' || fileTypeName === 'GIF') {
            ...
            //   name: '图片预览',
            //   query: {
            //     imgSrcBig: process.env.API_OS_URL + "/efOS/file/getImgByUrl?fileId=" + row.proofUrl + '&fileUrl=exhibitor_amend'
            //   }
            // });
            // window.open(routeData.href, '_blank', 'height=800,width=500');
          }
          // else if (fileTypeName === 'pdf' || fileTypeName === 'PDF') {
          //   this.isImg = false;
          //   // this.previewHref = process.env.API_OS_URL + '/efOS/file/queryFile?fileId=' + row.proofUrl;
          //   // this.previewHref = process.env.API_OS_URL + "/efOS/file/getImgByUrl?fileId=" + row.proofUrl + '&fileUrl=exhibitor_amend';
          //   // window.open(this.previewHref);
      ...
      if (row.colname === 'company_name' || row.colname === 'enterprise_abbreviation') {
        ...
        this.$confirm('您是否确认审核通过?', '提示', {
          ...
          this.getreviewCompanyImportInfo(param).then(res => {
            ...
            // 初始列表
            this.getEnterImportCompanyInfo(this.searchParams).then(reso => {
    ...
    batchAuditPass(row) {
      ...
      this.batchAuditCompanyNameAndAbbreviation(param).then(res => {
        ...
        this.auditTableResultDialogVisible = false;
        // 初始列表
        this.getEnterImportCompanyInfo(this.searchParams).then(response => {
    ...
    toSearch() {
      ...
      this.searchParams.companyImportColumnSession = currentObject;
      this.searchParams.companyImportColumnSession.businessDelegationId = this.delegationId;
      // 查询时默认设置起始页为1
