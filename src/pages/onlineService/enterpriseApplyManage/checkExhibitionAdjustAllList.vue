        ...
        <el-table-column align="center" label="操作" width="200">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              size="small"
            >审核</el-button>
            <!-- <el-button :disabled="scope.row.businessDelegationStatus === '1'" @click="handleNotgo(scope.row)" type="text" size="small">不通过</el-button> -->
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...mapActions("companyInfomation", ["getReviewAreaAdjustment", "postReviewAreaAdjustmentBatch"]),
      ...
      this.getReviewAreaAdjustment(values)
        .then(res => {
          ...
          for (let i = 0; i < skb.length; i++) {
            if (skb[i].businessDelegationStatus === "0") {
              arrs.push(skb[i]);
            }
          }
          this.tableData = arrs;
    ...
    checkboxInit(row, index) {
      if (row.businessDelegationStatus === "1") {
        return false;
      } else {
        return true; //可勾选
      }
      ...
      if (rows) {
        rows.forEach(row => {
          ...
          if (row) {
            ...
            searchParamPass.businessDelegationStatus = "1";
    ...
    handlePass(val) {
      ...
      this.$confirm('请确认执行该操作', '提示', {
        ...
        if (this.getList && val === "pass") {
          ...
          this.getList.businessDelegationStatus = 2;
          this.postReviewAreaAdjustmentBatch(this.getList).then(() => {
            this.handleClickData(this.searchParams);
            this.$message({
              type: 'success',
