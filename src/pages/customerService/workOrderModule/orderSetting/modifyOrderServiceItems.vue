...
<template>
  <el-tabs v-model="activeName" @tab-click="handleClick">
    <el-tab-pane label="修改工单服务项目" name="three">
...
export default {
  ...
  created() {
    ...
    let _this = this;
    this.loading = true;
    this.selectWorkProject(this.id).then(response => {
  ...
  // },
  methods: {
    ...mapActions('orderSetting', ['selectWorkProject', 'updateAcceptorHallInfo']),
    getQuery() {
      // 取到路由带过来的参数 
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // 新增
          this.updateAcceptorHallInfo(this.ruleForm).then(res => {
            this.$message({
              type: 'success',
              message: '修改成功'
            });
