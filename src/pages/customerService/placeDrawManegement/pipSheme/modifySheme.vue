...
<template>
  <el-tabs v-model="activeName" @tab-click="handleClick">
    <el-tab-pane label="修改标准展位展具配置" name="three">
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        configurationDiagram: [{ required: true, message: "请选择", trigger: "change" }],
        ifOverlap: [{ required: true, message: "请选择", trigger: "change" }],
        ifLaminate: [{ required: true, message: "请选择", trigger: "change" }],
        laminateRange: [{ required: true, message: "请输入", trigger: "blur" }],
        ifFasten: [{ required: true, message: "请选择", trigger: "change" }],
        ifSee: [{ required: true, message: "请选择", trigger: "change" }],
        effectDraw: [{ required: true, message: " ", trigger: "blur" }],
        buildLegend: [{ required: true, message: " ", trigger: "blur" }],
        removeLegend: [{ required: true, message: " ", trigger: "blur" }],
        applicationArea: [{ required: true, message: "请选择", trigger: "change" }],
  ...
  methods: {
    ...mapActions('sheme', ['selectListByPage', 'selectById', 'updateByIdInfo']),
    getQuery() {
      // 取到路由带过来的参数 
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // 新增
          this.updateByIdInfo(this.ruleForm).then(res => {
            this.$message({
              type: 'success',
              message: '修改成功'
            });
