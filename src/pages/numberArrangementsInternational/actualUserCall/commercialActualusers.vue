<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="商协会复核" name="first" v-loading.fullscreen="fullLoading">
      <!-- 表单 -->
      ...
      <el-table :data="tableData" style="width: 100%" border>
        ...
        <el-table-column align="center" label="交易团审核" prop="supportTotle">
          ...
          <el-table-column align="center" min-width="120" label="是否符合广交会参展资格标准" prop="exhibitsFlagFromDeal">
            <template slot-scope="scope">
              <span>{{scope.row.exhibitsFlagFromDeal | status}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" min-width="120" label="不合格原因说明" prop="examineOpinionFromDeal"></el-table-column>
        </el-table-column>
        <el-table-column align="center" label="商协会复核" prop="boothTotal">
          ...
          <el-table-column align="center" min-width="120" label="出口额（万美元）" prop="exportFlag">
            <template slot-scope="scope">
              <el-input v-model.trim="scope.row.exportFlag" v-if="!(scope.row.examineStatus === '0' || scope.row.examineStatus === '1' || scope.row.examineStatus === '2')"></el-input>
          ...
          <el-table-column align="center" min-width="120" label="是否符合广交会参展资格标准" prop="examineStatus">
            <template slot-scope="scope">
              <div v-if="scope.row.examineStatus == null || scope.row.examineStatus === ''">
...
export default {
  ...
  methods: {
    ...mapActions("actualUserCall", [
      "getlistCoceralGrade", // 商协会复核(分页)
      "getlistDealCondition", // 查询条件
      "addcoceralGrade" // 点击“通过、不通过或者未审核”按钮，商协会提交审核参展资格
    ]),
    ...
    // 获取页面数据
    getListPage(Url) {
      this.getlistCoceralGrade(Url).then(res => {
        ...
        //   this.tableData[i].charterFlag = "";
        //   this.tableData[i].companyFlag = "";
        //   this.tableData[i].exportFlag = "";
        //   // this.tableData[i].exhibitsFlag = "";
        //   this.tableData[i].blacklistFlag = "";
        // }
      }).catch(e => {});
    },
    // 关闭弹框
    ...
    handPass(val, num) {
      ...
      if (num === 1) {
        this.$confirm('该参展展品符合广交会参展资格标准', '提示', {
          ...
          this.addcoceralGrade(parame).then(res => {
            this.getListPage(this.obj);
            this.ynPass = true;
            this.$message({
              type: 'success',
    ...
    handNopass() {
      ...
      if (param.isJoinCecf === "") {
        ...
        this.addcoceralGrade(param).then(res => {
          this.getListPage(this.obj);
          this.formDad.noreson = "";
          this.ynPass = true;
          this.$message({
