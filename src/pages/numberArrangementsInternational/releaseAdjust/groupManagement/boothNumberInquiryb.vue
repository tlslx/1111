<template>
  ...
  <!--<el-card style="padding-bottom: 24px">-->
    ...
    <el-table :data="tableData" style="width: 100%" size="small" border :summary-method="getSummaries">
      ...
      <el-table-column align="center" min-width="76" label="商会" prop="coceralName"></el-table-column>
      <el-table-column align="center" min-width="160" label="展区" prop="exhibitionAreaName"></el-table-column>
      <el-table-column align="center" min-width="76" :label="`${getCes}届初始展位数`" prop="boothNumberBefore"></el-table-column>
      <el-table-column align="center" min-width="76" label="等量调换" prop="equalExchangeNumber">
        <template slot-scope="scope">
          {{scope.row.equalExchangeNumber == null ? "0" : scope.row.equalExchangeNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="不等量调换" prop="unequalExchangeNumber">
        <template slot-scope="scope">
          {{scope.row.unequalExchangeNumber == null ? "0" : scope.row.unequalExchangeNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="还原调整" prop="restoreExchangeNumber">
        <template slot-scope="scope">
          {{scope.row.restoreExchangeNumber == null ? "0" : scope.row.restoreExchangeNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="退展位" prop="refundNumber">
        <template slot-scope="scope">
          {{scope.row.refundNumber == null ? "0" : scope.row.refundNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="新增展位" prop="newExhibitionNumber">
        <template slot-scope="scope">
          {{scope.row.newExhibitionNumber == null ? "0" : scope.row.newExhibitionNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="回收展位" prop="recoveryNumber">
        <template slot-scope="scope">
          {{scope.row.recoveryNumber == null ? "0" : scope.row.recoveryNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="展位类型调整" prop="typeExchangeNumber">
        <template slot-scope="scope">
          {{scope.row.typeExchangeNumber == null ? "0" : scope.row.typeExchangeNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="其他调整" prop="otherNumber">
        <template slot-scope="scope">
          {{scope.row.otherNumber == null ? "0" : scope.row.otherNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" :label="`${getCes}届最终展位数`" prop="boothNumberFinal"></el-table-column>
    </el-table>
    <!-- 一堆按钮 -->
...
export default {
  ...
  methods: {
    ...
    // 下载
    handDownload() {
      let url = "delegationId=" + this.obj.delegationId + "&boothType=" + this.formData.boothType + "&exhibitionPeriod=" + this.formData.exhibitionPeriod + "&exhibitionAreaCode=" + this.formData.exhibitionAreaCode + "&coceralId=" + this.formData.coceralId;
