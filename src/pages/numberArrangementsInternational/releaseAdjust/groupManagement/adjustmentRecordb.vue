<template>
  <div v-loading.fullscreen="fullLoading">
    ...
    <el-table :data="tableData" style="width: 100%" border size="small">
      ...
      <el-table-column align="center" min-width="76" label="展位类型" prop="boothType">
        <template slot-scope="scope">
          {{scope.row.boothType === "1" ? "一般性展位" : "中央通道展位"}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="编号" prop="serialNumber"></el-table-column>
      <el-table-column align="center" min-width="76" label="录入日期" prop="createDate">
        <!-- <template slot-scope="scope">
          {{scope.row.createDate | credat}}
        </template> -->
      </el-table-column>
      <el-table-column align="center" min-width="76" label="交易团" prop="delegationName"></el-table-column>
      <el-table-column align="center" min-width="76" label="展区" prop="exhibitionAreaName"></el-table-column>
      <el-table-column align="center" min-width="76" label="与之互调的交易团" prop="adjustDelegationName"></el-table-column>
      <el-table-column align="center" min-width="76" label="调整展位数" prop="adjustNumber">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" min-width="76" label="调整类型" prop="adjustType">
        <template slot-scope="scope">
          {{scope.row.adjustType | adjustTypeAdjust}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="计入基数" prop="isJoinBase">
        <template slot-scope="scope">
          {{scope.row.isJoinBase === "YES" ? "是" : "否"}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="备注" prop="remark"></el-table-column>
...
export default {
        ...
        customQueryParams: [
          {
            ...
            joinType: "And",
            dataType: "Default",
            values: [this.$store.getters.userInfo.org.deptId],
            description: ""
          }
        ]
      },
  ...
  methods: {
    ...
    // 下载
    handDownload() {
      let url = "exhibitionSession=" + this.formData.exhibitionSession + "&boothType=" + this.formData.boothType + "&adjustDelegationId=" + this.formData.adjustDelegationId + "&delegationId=" + this.$store.getters.userInfo.org.deptId + "&exhibitionAreaCode=" + this.formData.exhibitionAreaCode + "&adjustType=" + this.formData.adjustType + "&boothSource=0";
