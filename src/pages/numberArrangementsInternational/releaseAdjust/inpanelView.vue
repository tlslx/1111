<template>
  ...
  <div v-loading.fullscreen="fullLoading">
    ...
    <el-form :inline="true" :model="formData" size="small">
      ...
      <el-form-item label="交易团推荐百分比" label-width="140px">
        <el-select v-model="formData.hundredPercentRecommend" clearable>
          <el-option label="100%" value="1"></el-option>
          <el-option label="非100%" value="0"></el-option>
        </el-select>
      </el-form-item>
    ...
    <!-- 表格 -->
    <el-table :data="tableData" style="width: 100%" border size="small" :summary-method="getSummaries">
      <!-- <el-table-column align="center" type="selection" width="55"></el-table-column> -->
      <el-table-column align="center" min-width="76" label="交易团" prop="delegationName"></el-table-column>
      <el-table-column align="center" min-width="76" label="展期" prop="exhibitionPeriod"></el-table-column>
      <el-table-column align="center" min-width="160" label="展区" prop="exhibitionAreaName"></el-table-column>
      <el-table-column align="center" min-width="76" label="展位数（不含卡车通道展位）" prop="ordinaryNumber">
        <template slot-scope="scope">
          {{scope.row.ordinaryNumber == null ? "0" : scope.row.ordinaryNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="卡车通道展位数" prop="centerChannelNumber">
        <template slot-scope="scope">
          {{scope.row.centerChannelNumber == null ? "0" : scope.row.centerChannelNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="最终展位数" prop="boothNumberFinal">
        <template slot-scope="scope">
          {{scope.row.boothNumberFinal == null ? "0" : scope.row.boothNumberFinal}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="交易团推荐特装数" prop="recommendSpecial">
        <template slot-scope="scope">
          {{scope.row.recommendSpecial == null ? "0" : scope.row.recommendSpecial}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="交易团推荐标摊数" prop="recommendStandard">
        <template slot-scope="scope">
          {{scope.row.recommendStandard == null ? "0" : scope.row.recommendStandard}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="交易团推荐卡车通道数" prop="recommendCentralChannel">
        <template slot-scope="scope">
          {{scope.row.recommendCentralChannel == null ? "0" : scope.row.recommendCentralChannel}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="交易团未推荐数" prop="notRecommendNumber">
        <template slot-scope="scope">
          {{scope.row.notRecommendNumber == null ? "0" : scope.row.notRecommendNumber}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="76" label="交易团推荐百分比" prop="recommendPercent">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" min-width="80" label="实际安排特装数" prop="actualRecommendSpecial">
        <template slot-scope="scope">
          {{scope.row.actualRecommendSpecial == null ? "0" : scope.row.actualRecommendSpecial}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="80" label="实际安排标摊数" prop="actualRecommendStandard">
        <template slot-scope="scope">
          {{scope.row.actualRecommendStandard == null ? "0" : scope.row.actualRecommendStandard}}
        </template>
      </el-table-column>
      <el-table-column align="center" min-width="80" label="实际安排卡车通道数" prop="actualRecommendCentralChannel">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    // 下载
    handDownload() {
      let url = this.paginationData.currentPage + "&size=" + this.paginationData.total + "&exhibitionAreaCode=" + this.formData.exhibitionAreaCode + "&exhibitionPeriod=" + this.formData.exhibitionPeriod + "&token=" + this.token;
