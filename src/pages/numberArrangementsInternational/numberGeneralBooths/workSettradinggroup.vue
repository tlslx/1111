<template>
  <div >
    ...
    <el-tab-pane label="工作部设定交易团一般性展位数" name="first">
      <el-card>
        <!-- <> -->
        <el-form :inline="true" :model="formInline" class="demo-form-inline" size="small">
          <el-form-item label="交易团">
            <el-select size="small" class="inpu" v-model="formInline.delegationName" clearable>
              <el-option v-for="item in delegation" :label="item.deptName" :value="item.deptId" :key="item.deptId">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="展区">
            <el-select size="small" class="inpu" v-model="formInline.exhibitionAreaCode" clearable>
              <el-option v-for="list in exhibitionAreaOpts" :label="list.exhibitionAreaName" :value="list.exhibitionAreaCode"
                :key="list.exhibitionAreaCode"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item class="but">
            <el-button type="primary" class="btut floatLeft" @click="handleQuery">查询</el-button>
            <!-- <el-upload
              class="impfloat floatLeft"
              action=""
              :limit="1"
              :http-request="uploadFileMethod">
              <el-button class="formStyle" size="small" type="primary">导入</el-button>
            </el-upload> -->
            <el-button class="formStyle" size="small" type="primary" @click="handUpload">导入</el-button>
          </el-form-item>
        </el-form>
        <el-table :data="getChanne.records" border style="width: 100%" ref="multipleTable" size="small" @select="handleSelectionChange"
          @select-all="handleSelectionDhange" @selection-change="handleCheckedChange" :row-key="getRowKey">
          <!-- <el-table-column type="selection" fixed width="55">
          </el-table-column> -->
          <el-table-column prop="delegationName" align="center" min-width="120" label="交易团">
          </el-table-column>
          <el-table-column prop="exhibitionAreaName" align="center" min-width="120" label="展区">
          </el-table-column>
          <el-table-column prop="exhibitionPeriod" align="center" min-width="120" label="展期">
          </el-table-column>
          <el-table-column prop="lastOrdinaryNumber" align="center" min-width="120" label="上届一般性展位数量">
            <template slot-scope="scope">{{ scope.row.lastOrdinaryNumber == null ? "0" : scope.row.lastOrdinaryNumber === "" ? "0" : scope.row.lastOrdinaryNumber }}</template>
          </el-table-column>
          <el-table-column prop="lastCenterChannelNumber" align="center" min-width="120" label="上届中央通道展位数">
            <template slot-scope="scope">{{ scope.row.lastCenterChannelNumber == null ? "0" : scope.row.lastCenterChannelNumber === "" ? "0" : scope.row.lastCenterChannelNumber }}</template>
          </el-table-column>
          <el-table-column prop="ordinaryNumber" align="center" min-width="120" label="一般性展位数量核定数">
            <template slot-scope="scope">{{ scope.row.ordinaryNumber == null ? "0" : scope.row.ordinaryNumber === "" ? "0" : scope.row.ordinaryNumber }}</template>
          </el-table-column>
          <el-table-column prop="centerChannelNumber" align="center" min-width="120" label="中央通道展位数">
            <template slot-scope="scope">{{ scope.row.centerChannelNumber == null ? "0" : scope.row.centerChannelNumber === "" ? "0" : scope.row.centerChannelNumber }}</template>
          </el-table-column>
        ...
        <el-dialog class="classapop" title="编辑" @close="maskLayer" :visible.sync="dialogFormVisible"
          v-if="dialogFormVisible" width="500px">
          <el-form :model="form" :rules="rules" ref="ruleForm" size="small" label-width="180px">
            <el-form-item label="一般性展位数量核定数：" prop="ordinaryNumber">
              <el-input v-model="form.ordinaryNumber" maxlength="8" class="inpu"></el-input>
            </el-form-item>
            <el-form-item label="中央通道展位数量：" prop="centerChannelNumber">
              <el-input v-model="form.centerChannelNumber" :disabled="disabled" maxlength="8"
                class="inpu"></el-input>
            </el-form-item>
          </el-form>
          <div slot="footer" class="dialog-footer">
            <el-button size="small" type="primary" @click="handelSure('ruleForm')" class="btut">确定</el-button>
            <el-button size="small" @click="handelCancel('form')" class="btut">取消</el-button>
          </div>
        </el-dialog>
        <el-button type="danger" size="small" class="btut" @click="handelPublish">公布</el-button>
        <el-button type="primary" size="small" class="btut" @click="handelDow">下载</el-button>
        <el-dialog class="classapop" title="公布" @close="maskPublish" :visible.sync="dialogFormVi" v-if="dialogFormVi"
          ...
          <div slot="footer" class="dialog-footer">
            <el-button size="small" type="primary" @click="handelEnsure" class="btut">确定</el-button>
            <el-button size="small" @click="handelAbolish" class="btut">取消</el-button>
          </div>
        </el-dialog>
        <el-form :inline="true" style="float: right;" size="small">
          <el-form-item>
            <el-pagination style="margin-top: 20px;" background layout="total, sizes, prev, pager, next, jumper"
              ...
              :current-page="getChanne.current" :page-size="getChanne.size" :page-sizes="pagaData.pageSizes">
            </el-pagination>
          </el-form-item>
        </el-form>
        <!-- 下载模板 -->
  ...
  export default {
    ...
    data() {
      let checkAge = (rule, value, callback) => {
        let regRule = /^\d*\.{0,1}\d{0,1}$/;
        if (!value) {
          return callback(new Error("输入不能为空"));
        } else if (!regRule.test(value)) {
          return callback(new Error("请输入正整数或一位小数"));
        }
        callback();
      };
      return {
        ...
        fileRule: {
          chooseFileName: [{ required: true, message: "", trigger: "change" }]
        },
        exhibitionAreaOpts: []
      };
    },
    ...
    computed: {
      ...
      formQuery() {
        return {
          delegationId: this.formInline.delegationName,
    ...
    methods: {
      ...mapActions("numberGeneralBooths", [
        "getChanneArrangements", // 获取页面
        "postChanneCopyreader",  // 编辑
        "postChanneGuideInto",   // 导入
        "publishsendInternalLetter" // 公示
      ]),
      ...
      uploadFileMethod(param) {
        ...
        this.postChanneGuideInto(formData).then(res => {
          this.$message({
            type: "success",
            message: "导入成功"
          });
      ...
      handelRedact(val) {
        ...
        if (val.centerChannelNumber == null || val.centerChannelNumber === "") {
          this.disabled = true;
          this.rules.ordinaryNumber = [{ validator: checkAge, trigger: "blur" }];
          this.rules.centerChannelNumber = [];
        } else {
          this.disabled = false;
          this.rules.ordinaryNumber = [{ validator: checkAge, trigger: "blur" }];
          this.rules.centerChannelNumber = [{ validator: checkAge, trigger: "blur" }];
        }
      ...
      handleUploadFile(param) {
        ...
        this.postChanneGuideInto(formData).then(() => {
          this.$nextTick(() => {
      ...
      // on-change
      handleChange(fil, filist) {
        this.fileForm.chooseFileName = fil.name;
      },
      // 上传
      toUpload(formName) {
      ...
      // 模板下载
      handleDownloadFile() {
        // let url = "delegationCode=" + this.formData.delegationCode + "&exhibitionAreaCode=" + this.formData.exhibitionAreaCode + "&exhibitionAreaName=" + this.formData.exhibitionAreaName;
        // url = url.replace(/undefined/g, "");
        // window.open(process.env.API_NA_URL + "/api/OrdinaryDemandQuantity/importCenterNumber/module/download?" + url + "&token=" + localStorage.getItem("USER_TOKEN"), "_parent");
        window.open(process.env.API_NA_URL + "/api/OrdinaryDemandQuantity/importCenterNumber/module/download" + "?token=" + localStorage.getItem("USER_TOKEN"), "_blank");
      },
      // 下载
      handelDow() {
        let url = "delegationCode=" + this.formQuery.delegationCode + "&exhibitionAreaCode=" + this.formQuery.exhibitionAreaCode + "&exhibitionAreaName=" + this.formQuery.exhibitionAreaName;
        url = url.replace(/undefined/g, "");
        //window.open(process.env.API_NA_URL + "/api/OrdinaryDemandQuantity/importCenterNumber/download?" + url + "&token=" + localStorage.getItem("USER_TOKEN"), "_parent");
        window.open(process.env.API_NA_URL + "/api/OrdinaryDemandQuantity/importCenterNumber/module/download?" + url + "&token=" + localStorage.getItem("USER_TOKEN"), "_parent");
      },
      // 公布
      handelPublish() {
      ...
      // 公布确定
      handelEnsure() {
        // this.getChanneArrangements({
        //   current: "1",
        //   size: this.getChanne.total
        // }).then(res => {
        //   this.publishsendInternalLetter().then(req => {}).catch(err => {});
        // }).catch(e => {});
        this.$message({
          type: "success",
          message: "公布成功！"
