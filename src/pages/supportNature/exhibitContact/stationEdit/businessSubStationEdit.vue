  ...
  <el-tabs v-model="activeName">
    <el-tab-pane label="查看" name="first">
      ...
      <el-table border :data="associationListData.records">
        ...
        <el-table-column prop="" label="操作">
          <template slot-scope="scope">
            <el-button type="text" @click="handleEditTwo(scope.row)" :disabled="scope.row.status !== 0">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-table border :data="stationListData.records">
        ...
        <el-table-column prop="" label="操作">
          <template slot-scope="scope">
            <el-button type="text" @click="handleEditThr(scope.row)" :disabled="scope.row.status !== 0">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-row type="flex" justify="end" class="pagination-contain" style="padding-bottom: 10px;">
      ...
      <div class="above-table">交易团秘书长以上参会人员信息
      ...
      <el-table border :data="generalListData.records">
        ...
        <el-table-column prop="" label="操作">
          <template slot-scope="scope">
            <el-button type="text" @click="handleEditFo(scope.row)" :disabled="scope.row.status !== 0">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-row type="flex" justify="end" class="pagination-contain" style="padding-bottom: 10px;">
...
export default {
  data() {
    return {
      activeName: 'first',
      twoVis: false, thrVis: false, foVis: false,
  ...
  computed: {
    ...mapGetters('exhibitContact', ['associationListData', 'stationListData', 'generalListData', 'withdrawStatus']),
    paramsTwo() {
      return {
  ...
  methods: {
    ...mapActions('exhibitContact', ['getAssociationList', 'getStationList', 'getGeneralList',
      'postTradingGroupAssociationEdit', 'postTradingGroupStationEdit', 'postSecretaryGeneralEdit',
    ...
    handleEditTwo(row) {
      let _this = this;
      this.handleEditCopy(_this.twoForm, row); this.twoVis = true;
    },
    handleEditThr(row) {
      let _this = this;
      this.handleEditCopy(_this.thrForm, row); this.thrVis = true;
    },
    handleEditFo(row) {
      let _this = this;
      this.handleEditCopy(_this.foForm, row); this.foVis = true;
    },
    handleWithDraw() { // 申请撤回
