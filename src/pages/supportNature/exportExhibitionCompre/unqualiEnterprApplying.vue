/**
* @file 各团各展区申请企业不合格原因查询
* @author:heli
* @date:2019/4/17
*/
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="各团各展区申请企业不合格原因查询" name="first">
      <el-form :inline="true" :model="boothParams" class="demo-ruleForm">
        <el-form-item label="届数:" size="mini">
          <!-- 届数下拉框 -->
          <el-select v-model="boothParams.exhibitionSession" placeholder="请选择" @change="onSelected">
        ...
        <el-form-item label="展区:" size="mini" v-show="isShow">
          <!-- 展区下拉框 -->
        ...
        <el-form-item label="企业名称:" size="mini" v-show="isShow">
          <!-- 企业名称下拉框 -->
          <el-input v-model="boothParams.companyName" clearable></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  mounted() {
    // 列表展示
    this.exhibitionApplyNoReason(this.boothParams).then(res => {
  ...
  methods: {
    ...
    ...mapActions("enterpriseBooth", ["exhibitionApplyNoReason"]), // 列表展示
    // 搜索框查询
    submitForm() {
      // 列表查询
      this.tableData = [];
    ...
    onSelected() {
      // 根据展届展期获取展区和专区的信息
      let params = {
        exhibitionNum: this.boothParams.exhibitionSession
      };

      this.getAreas(params).then(res => {
        this.ruleForm.exhibitionAreaArr = res;

        if (this.boothParams.exhibitionSession === '') {
    ...
    // 高级查询
    advancedQuery() {
      this.boothParams.delegationId = '';
      this.boothParams.companyName = '';
      this.boothParams.exhibitionAreaCode = '';
   ...
   */
    ...
    downLoad() {
      let downLoadParams = {
        applyYear: this.boothParams.applyYear,
        delegationId: this.boothParams.delegationId,
        companyName: this.boothParams.companyName,
        exhibitionAreaCode: this.boothParams.exhibitionAreaCode,
        exhibitionSession: this.boothParams.exhibitionSession
      };
