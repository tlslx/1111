...
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="退展位情况" name="first">
      <el-form :inline="true" :model="ruleForm" class="demo-ruleForm">
        ...
        <el-form-item label="退回大会一般性性展位数量:" size="mini" label-width="184px">
          <el-input v-model="ruleForm.returnNumberValue" placeholder="0" clearable></el-input>
        </el-form-item>
      ...
      <el-dialog class="modifyReach" title="设置达标标准" :visible.sync="dialogVisibleSet" width="30%" :before-close="handleClose">
        <el-form :model="form">
          <el-form-item label="退回大会一般性性展位数量" label-width="180px">
            <el-input v-model="form.returnNumberValue" type="text" placeholder="请输入展位数量"></el-input>
          </el-form-item>
        </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      logoUploadUrl: process.env.API_REPORT_URL + '/api/efSupBoothNumberReturnSta/import'
    };
  },
  mounted() {
    // 退展位情况 查询
    this.efSupBoothStaPage(this.boothParams).then(res => {
    ...
    // 获取标准值
    let standingParams = {
      type: 'NumberReturnSta'
    };

    this.efSupBoothStaStanding(standingParams).then(res => {
      res.records.forEach(item => {
        if (item.item === "returnNumberValue") {
          this.form.returnNumberValue = item.value;
          this.ruleForm.returnNumberValue = item.value;
        }
      });
    });
  },
  methods: {
    ...
    ...mapActions("enterpriseBooth", ["efSupBoothStaEdit"]), // 退展位情况 修改
    ...mapActions("enterpriseBooth", ["efSupBoothStaPage"]), // 退展位情况 查询
    ...mapActions("enterpriseBooth", ["efUpdateStand"]), // 退展位情况 设置标准值
    ...mapActions("enterpriseBooth", ["searchBusinessDelegation"]), // 获取交易团
    ...mapActions("enterpriseBooth", ["efSupBoothStaStanding"]), // 退展位情况 获取标准值
    ...
    uploadSuccess(res, file) {
      if (res.flag === "0") {
        ...
        // 企业一般性特装展位数调整情况达标 查询
        this.efSupBoothStaPage(this.boothParams).then(resp => {
    ...
    setStandarding() {
      let params = {
        'type': 'NumberReturnSta',
        'returnNumberValue': this.form.returnNumberValue
      };
      ...
      // 设置标准值
      this.efUpdateStand(params).then(res => {
        // 获取标准值
        if (res > 0) {
          this.efSupBoothStaStanding(standingParams).then(respon => {
    ...
    handleEdit(index, row) {
      ...
      this.formManual.boothNumberReturnId = row.boothNumberReturnId;
    },
    // 确定 修改
    confirmManual() {
      this.tableData = []; // 清空数组

      this.efSupBoothStaEdit(this.formManual).then(res => {
        this.efSupBoothStaPage(this.boothParams).then(resp => {
          this.total = resp.total;
   ...
   */
    ...
    downLoad() {
      ...
      kindo.util.getdown(`${process.env.API_REPORT_URL}/api/efSupBoothNumberReturnSta/download?${this.encodeSearchParams(downLoadParams)}`, '交易团退展位情况.xls');
    }
  }
};
</script>
