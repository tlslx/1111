/**
* @file 企业一般性特装展位数调整情况达标
* @author:heli
* @date:2019/5/7
*/
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="企业一般性特装展位数调整情况达标" name="first">
      <el-form :inline="true" :model="ruleForm" class="demo-ruleForm">
        ...
        <el-form-item label="调整占本团参展企业数比例<" size="mini" label-width="190px">
          <el-input v-model="ruleForm.changeRate" placeholder="100" class="numStandard" clearable></el-input>
          <span class="numPercent">%</span>
        </el-form-item>

        <el-form-item label="特装调整次数<" size="mini" label-width="190px">
          <el-input v-model="ruleForm.changeNumber" placeholder="请输入特装调整次数" clearable></el-input>
        </el-form-item>
      ...
      <el-dialog class="modifyReach" title="设置达标标准" :visible.sync="dialogVisibleSet" width="30%" :before-close="handleClose">
        <el-form :model="form">
          <el-form-item label="调整占本团参展企业数比例<" size="mini" label-width="190px">
            <el-input v-model="form.changeRate" placeholder="100" class="numStandard" clearable></el-input>
            <span class="perNumPercent">%</span>
          </el-form-item>

          <el-form-item label="特装调整次数<" size="mini" label-width="190px">
            <el-input v-model="form.changeNumber" placeholder="请输入特装调整次数" clearable></el-input>
          </el-form-item>
        </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      logoUploadUrl: process.env.API_REPORT_URL + '/api/efSupChangeSta/import'
    };
  },
  mounted() {
    // 企业一般性特装展位数调整情况达标 查询
    this.efSupChangeStaSearch(this.boothParams).then(res => {
    ...
    // 获取标准值
    let standingParams = {
      type: 'NumberChangeSta'
    };

    this.efGetStanding(standingParams).then(res => {
      res.records.forEach(item => {
        if (item.item === "changeRateValue") {
          this.form.changeRate = item.value;
          this.ruleForm.changeRate = item.value;
        }

        if (item.item === "changeNumberValue") {
          this.form.changeNumber = item.value;
          this.ruleForm.changeNumber = item.value;
        }
      });
    });
  },
  methods: {
    ...
    ...mapActions("enterpriseBooth", ["efSupChangeStaEdit"]), // 修改
    ...mapActions("enterpriseBooth", ["searchBusinessDelegation"]), // 获取交易团
    ...mapActions("enterpriseBooth", ["efEditStand"]), // 企业一般性特装展位数调整情况达标 设置标准值
    ...mapActions("enterpriseBooth", ["efGetStanding"]), // 企业一般性特装展位数调整情况达标 获取标准值
    ...mapActions("enterpriseBooth", ["efSupChangeStaSearch"]), // 企业一般性特装展位数调整情况达标 查询
    ...
    uploadSuccess(res, file) {
      if (res.flag === "0") {
        ...
        // 企业一般性特装展位数调整情况达标 查询
        this.efSupChangeStaSearch(this.boothParams).then(resp => {
    ...
    setStandarding() {
      let params = {
        'type': 'NumberChangeSta',
        'changeRateValue': this.form.changeRate,
        'changeNumberValue': this.form.changeNumber
      };
      ...
      // 设置标准值
      this.efEditStand(params).then(res => {
        // 获取标准值
        if (res > 0) {
          this.efGetStanding(standingParams).then(respon => {
    ...
    handleEdit(index, row) {
      ...
      this.formManual.orgId = row.orgId;
      this.formManual.modifyReason = row.modifyReason;
      this.formManual.exhibitionNum = row.exhibitionNum;
      this.formManual.boothNumberChangeId = row.boothNumberChangeId;
    },
    // 确定 修改
    confirmManual() {
      this.tableData = []; // 清空数组

      this.efSupChangeStaEdit(this.formManual).then(res => {
        this.efSupChangeStaSearch(this.boothParams).then(resp => {
          this.total = resp.total;
   ...
   */
    ...
    downLoad() {
      ...
      kindo.util.getdown(`${process.env.API_REPORT_URL}/api/efSupChangeSta/download?${this.encodeSearchParams(downLoadParams)}`, '企业一般性特装展位数调整情况达标.xls');
    }
  }
};
</script>
