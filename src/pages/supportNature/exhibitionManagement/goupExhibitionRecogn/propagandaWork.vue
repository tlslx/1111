...
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="专业化宣传工作情况" name="first">
      <el-form :inline="true" :model="ruleForm" class="demo-ruleForm">
        ...
        <el-form-item label="积极宣传展区宣传素材:" size="mini" label-width="170px">
          <el-input v-model="ruleForm.propagadaValue" disabled></el-input>
        </el-form-item>
      ...
      <template>
        <el-table
          ...
          <el-table-column
            prop="isMaterialProvided"
            label="是否积极宣传展区宣传素材"
            align="center">
            <template slot-scope="scope">
              <span v-if="scope.row.isMaterialProvided==0">否</span>
              <span v-else-if="scope.row.isMaterialProvided==1">是</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="isPassed"
            label="组展表彰指标-提供素材达标情况"
            align="center">
            <template slot-scope="scope">
      ...
      <el-dialog class="modifyReach" title="设置达标标准" :visible.sync="dialogVisibleSet" width="30%" :before-close="handleClose">
        <el-form :model="propForm">
          <el-form-item label="积极宣传展区宣传素材:" size="mini" label-width="210px">
            <el-select v-model="propForm.propagadaValue" placeholder="请选择">
...
export default {
  ...
  data() {
    return {
      ...
      logoUploadUrl: process.env.API_REPORT_URL + '/api/efSupPropagandaInfoSta/import'
    };
  },
  mounted() {
    // 专业化宣传工作情况 查询
    this.efSupPropagandaInfoSta(this.boothParams).then(res => {
    ...
    this.proGetStanding(standingParams).then(res => {
      res.records.forEach(item => {
        if (item.item === "propagadaValue") {
          if (item.value === "1") {
            this.ruleForm.propagadaValue = '是';
          } else {
            this.ruleForm.propagadaValue = '否';
          }
        }
  ...
  methods: {
    ...mapActions("enterpriseBooth", ["proGetStanding"]), // 获取标准值
    ...mapActions("enterpriseBooth", ["getListAll"]), // 获取所有届数信息
    ...mapActions("enterpriseBooth", ["proUpdateStandEdit"]), // 修改标准值
    ...mapActions("enterpriseBooth", ["proUpdateStandStand"]), // 设置标准值
    ...mapActions("enterpriseBooth", ["searchBusinessDelegation"]), // 获取交易团
    ...mapActions("enterpriseBooth", ["efSupPropagandaInfoSta"]), // 专业化宣传工作情况 查询
    ...
    uploadSuccess(res, file) {
      if (res.flag === "0") {
        ...
        // 专业化宣传工作情况 查询
        this.efSupPropagandaInfoSta(this.boothParams).then(resp => {
    ...
    setStandarding() {
      let params = {
        'type': 'PropagandaInfo',
        'propagadaValue': this.propForm.propagadaValue
      };
      ...
      this.proUpdateStandStand(params).then(res => {
        // 获取标准值
        if (res > 0) {
          this.proGetStanding(standingParams).then(respon => {
            if (this.propForm.propagadaValue === "1") {
              this.ruleForm.propagadaValue = '是';
            } else {
              this.ruleForm.propagadaValue = '否';
            }
    ...
    handleEdit(index, row) {
      ...
      this.formManual.propagandaId = row.propagandaId;
    },
    // 确定 修改
    confirmManual() {
      this.proUpdateStandEdit(this.formManual).then(res => {
        this.efSupPropagandaInfoSta(this.boothParams).then(resp => {
          this.total = resp.total;
          this.tableData = resp.records;
        });
   ...
   */
    ...
    downLoad() {
      ...
      kindo.util.getdown(`${process.env.API_REPORT_URL}/api/efSupPropagandaInfoSta/download?${this.encodeSearchParams(downLoadParams)}`, '专业化宣传工作情况.xls');
    }
  }
};
</script>
