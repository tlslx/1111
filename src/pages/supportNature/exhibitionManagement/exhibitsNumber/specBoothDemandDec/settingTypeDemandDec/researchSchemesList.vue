...
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="调研方案列表" name="first">
      <el-form :inline="true" :model="boothParams" class="researchSchemeForm">
        ...
        <el-form-item label="调查方案名称:" size="mini">
          <!-- 调查方案名称下拉框 -->
          <el-input v-model="boothParams.surveyName" clearable></el-input>
        </el-form-item>
      ...
      <template>
        <el-table
          ...
          <el-table-column fixed="right"
            ...
            width="130">
            <template slot-scope="scope">
              <el-button @click="boothsNumber(scope.$index, scope.row)" type="text" size="small">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-button type="primary" @click="locationAdd" size="small" class="addExhibition">新增</el-button>
    </el-tab-pane>
  </el-tabs>
</template>
...
export default {
  ...
  mounted() {
    // 列表展示
    this.getWorkPage(this.boothParams).then(res => {
  ...
  methods: {
    ...mapActions("enterpriseBooth", ["getListAll", // 获取所有届数信息
      "searchBusinessDelegation", // 获取交易团
      "findDicItemListByExhibiArea", // 根据字典获取展区
      "getWorkPage"]), // 列表展示
