/**
 * @file 组展进度设置---组展单位-查看组展进度计划
 */
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="组展单位-查看组展进度计划" name="first">
      <div class="top-btn">
        <div class="top-session">
          ...
          届广交会出口展组展工作时间进度计划表
           <el-select v-model="isOnly"  class="top-select" @change="allChange">
      ...
      <default-table class="table-margin" :cols="cols" :data="tableData" :loading="loading" :stripe="stripe"></default-table>
...
export default {
  ...
  async created() {
    ...
    // this.sessionOption = Array.from(new Set(exhArray.concat(this.session))).sort((a, b) => a - b);
  ...
  methods: {
    ...mapActions('organizationProgress', ['currentSessionInfo', 'queryExhibitionNumInfo', 'modifyAfterPublishInfo', 'queryOrganizerWatchInfo']),
    toDownload() {
      kindo.util.getdown(`${process.env.API_SN_URL}/api/progress/see/org/export?exhibitionNum=${this.session}`, '查看组展进度计划(组展单位).xls');
    },
    // 初始列表
    getData() {
      this.loading = true;
      this.queryParams.exhibitionNum = this.session;
      this.queryParams.isOnly = this.isOnly;
      this.queryOrganizerWatchInfo(this.queryParams).then(res => {
        // 查询成功
        // 分页
        if (res.total > 0) {
