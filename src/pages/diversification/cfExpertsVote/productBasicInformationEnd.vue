    ...
    <el-tab-pane label="产品基本信息" name="first">
      <section class="top">
        <el-row class="item-row">
          <el-col :span="8">
            <span v-for="(category,index) in productCategorys"
                  v-if="category.productCategoryId== productBase.productCategoryId"
                  :key="index">{{category.productCategoryNameCn}}：</span>
            <span v-for="(votelimit,index1) in voteLimits" :key="index1" v-if="votelimit.productCategoryId== productBase.productCategoryId">
        ...
        <el-row :gutter="20" class="item-img">
          <el-col :span="10" class="bigimg_con">
            <imageMagnify
          ...
          <el-col :span="14">
            ...
            <el-row class="item-center">
              ...
              <el-button style="padding: 10px 30px;" type="warning" @click="scoreEdit()" v-if="reviewState==='1'">评分</el-button>
              <el-button style="padding: 10px 30px;" type="info" v-else>已评分</el-button>
      ...
      <section class="productInfo">
        <el-row :gutter="50">
          ...
          <el-col :span="24">
            ...
            <a target="_Blank"  style="display: inline-block;" v-for="(attachment,index) in productAttachments" v-if="attachment.noteType === '03'"  class="item-text" :href="showSmallImg(productAttachments[index].filePath)">{{productAttachments[index].fileName}}</a>
          </el-col>
      ...
      <!--评分弹窗-->
      <el-dialog title="打分" :visible.sync="scoreDialog" :close-on-click-modal="false" width="300px">
        ...
        <div class="dialog-footer">
          <el-button type="primary" @click="handMark">确 定</el-button>
...
<script>
  ...
  export default {
    name: "productBasicInformationEnd",
    components: {
      imageMagnify
    },
    data() {
      return {
        ...
        // baseApi: 'http://192.168.1.121:9014'
      };
    },
    mounted() {
    ...
    methods: {
      ...
      getCategoryVoteLimit(activityCode) {
        this.$http.get(this.baseApi + '/api/customservice/cf/listFinalProductVoteCount', {
      ...
      getActivityList() {
        this.$http.get(this.baseApi + '/api/customservice/cf/getCfActivityCode').then(res => {
          //this.$http.get(this.baseApi + '/api/customservice/cf/listCfActivity').then(res => {
      ...
      getProductCategoryInfo(activityCode) {
        this.$http.get(this.baseApi + '/api/customservice/cf/listProductCategory', {
          activityCode: activityCode
        }).then(res => {
      ...
      getProductDetailInfo() {
        this.$http.get(this.baseApi + '/api/customservice/cf/getCfProduct', {
      ...
      // 投票操作
      vetoProduct() {
        this.$http.post(this.baseApi + '/api/customservice/cf/firstReviewVote', { awardProductId: this.awardProductId }).then(res => {
          this.$message({
            message: '投票成功',
            type: 'success'
          });
      ...
      // 发送点评信息,专家评价接口
      sendCommentRequest() {
        this.$http.postJson(this.baseApi + '/api/customservice/cf/commentFinalProduct', {
          awardProductId: this.awardProductId,
      ...
      handMark() {
        let params = [{
          score: this.score,
          productCategoryId: this.productBase.productCategoryId,
          activityCode: this.productBase.activityCode,
          awardProductId: this.productBase.awardProductId
        }];
        this.$http.postJson(this.baseApi + '/api/customservice/cf/finalReviewVote', params).then(res => {
          this.$message({
            type: "success",
            message: "评分成功"
          });
      ...
      sendCancelVeto() {
        this.showDialogTips("确认撤销投票吗?").then(() => {
          this.$http.post(this.baseApi + '/api/customservice/cf/firstReviewVoteCancel', { reviewIds: this.reviewId }).then(res => {
      ...
      // 下一个产品
      nextProductVeto() {
        this.$http.post(this.baseApi + '/api/customservice/cf/nextFinalReviewVote', { productCategoryId: this.productBase.productCategoryId, awardProductId: this.productBase.awardProductId, votedFlag: this.votedFlag }).then(res => {
