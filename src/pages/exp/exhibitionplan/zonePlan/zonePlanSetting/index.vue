<template>
  <el-page ref="zonePlanSetting">
    <template slot="header">
      <child-dialog @reload="onResetQuery" ref="childDialog" :operational="operational"/>
      <!-- 列表查询 -->
      <kindo-box :title="source.language.zonePlanSetting" icon="fa-search">
       ...
       <el-page-data>
        <kindo-box>
          <kindo-table :cell-style="isCenter" ref="zonePlanTable" :queryParam="form" :url="api.get">
            ...
            :formatter="(row,colums)=>{return kindo.dictionary.getLabel(source.schemeStatus,row.schemeStatus)}" min-width="120" show-overflow-tooltip>
            </el-table-column>
            <el-table-column :label="source.language.operation" fixed="right" min-width="200" align="center">
              <template slot-scope="scope">
                <el-button v-if="right('edit')" type="text" @click="onAction('edit',scope.row)"
               v-show="scope.row.schemeStatus !== '3'">{{source.language.edit}}</el-button>
