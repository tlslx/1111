<template>
  <el-container>
    <el-dialog :visible.sync="reviewVisible" :close-on-click-modal="false" :close-on-press-escape="false" :fullscreen="isFullScreen"
      width="90%">
      <template slot="title">
        <span class="el-dialog__title">{{source.language.suggestedReview}}</span>
      ...
      <kindo-box>
        <kindo-table :cell-style="isCenter" ref="reviewTable" :pagination="false">
            ...
            <el-table-column prop="seaTariffNumber.revisionType" :label="source.language.category" min-width="120" :formatter="(row,colums)=>{return kindo.dictionary.getLabel(source.revisionType,row.seaTariffNumber.revisionType)}" show-overflow-tooltip></el-table-column>
      ...
      <kindo-box :title="source.language.opinion">
       ...
       <kindo-box>
        <el-form :model="opinionForm" ref="opinionForm" :rules="rules" label-width="60px">
      ...
      onsubmit(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.postJson(this.api.reject, this.rowData).then(res => {
              this.$message({
                type: 'success',
