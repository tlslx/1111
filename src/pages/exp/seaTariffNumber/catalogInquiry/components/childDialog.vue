    ...
    <el-dialog :visible.sync="childVisible" :close-on-press-escape="false" :close-on-click-modal="false" width="65%">
      ...
      <el-form :model="child" ref="child" label-width="130px" label-position="right" inline :disabled="action === 'view'" :rules="rules">
        ...
        <el-form-item :label="source.language.commodityCode" prop="code">
          <el-input v-model="child.code" @blur="codeChange" class="normalclass"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    getChild() {
      ...
      if (this.action === 'view') {
        this.$http.get(this.api.getEffective, { id: this.rowData.id }).then(res => {
          res.areaCode = [];
          ...
          if (this.child.seaTariffNumberAreas) {
            this.child.seaTariffNumberAreas.forEach(el => {
