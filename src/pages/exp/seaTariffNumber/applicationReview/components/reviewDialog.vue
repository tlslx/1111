<template>
  <el-container>
    <el-dialog :visible.sync="reviewVisible" :close-on-click-modal="false" :close-on-press-escape="false" :fullscreen="isFullScreen"
      width="90%">
      <template slot="title">
        <span class="el-dialog__title">{{source.language.review}}</span>
        <i class="iconfont icon-zuidahua" @click="onChangeSize" />
      </template>
      <kindo-box>
        <kindo-table :cell-style="isCenter" ref="reviewTable" :pagination="false">
            ...
            <el-table-column prop="numberEffective.changeDescription" :label="source.language.changeDescription" min-width="160" show-overflow-tooltip></el-table-column>
            <el-table-column prop="numberEffective.originCode" :label="source.language.originalCode" header-align="center" min-width="100" show-overflow-tooltip>
            </el-table-column>
      ...
      <kindo-box :title="source.language.opinion">
        <kindo-table :cell-style="isCenter" ref="opinionTable" :pagination="false" :max-height="400">
          ...
          <!-- <el-table-column prop="actName" :label="source.language.operation" min-width="160" show-overflow-tooltip header-align="center"></el-table-column> -->
       ...
       <kindo-box>
        <el-form :model="opinionForm" ref="opinionForm" :rules="rules" label-width="150px">
          ...
          <el-form-item :label="source.language.explain" prop="opinion">
            <el-input type="textarea" :rows="4" :placeholder="source.language.inputPlaceholder" v-model="opinionForm.opinion" style="width: 50%" @blur="(ev)=>ev.target.value = ev.target.value.trim()"></el-input>
          </el-form-item>
          <el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rowData: {},
        opinionForm: {
          auditResult: 'false', // 审核意见选项 
          opinion: ''      // 审核意见说明，拒绝时必填
        },
        rules: {
          auditResult: [], 
          opinion: []  
    ...
    async created() {
      ...
      this.rules.auditResult = [
        { required: true, message: this.source.language.selectPlaceholder + this.source.language.results, trigger: "change" }
      ];
      this.rules.opinion = [
    ...
    methods: {
      getReview() {
        this.$nextTick(function () {
          ...
          this.$http.get(this.api.getReviewData, { id: this.rowData.id }).then(res => {
            let dataset = [];
            dataset.push(res);
      ...
      onsubmit(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.rowData.numberEffective = { code: this.rowData.numberEffective.code };
            // this.opinionForm.id = this.rowData.id;
            // this.opinionForm.taskId = this.rowData.taskId;
            // this.opinionForm.actId = this.rowData.actId;
            // this.opinionForm.numberEffective = { code: this.rowData.numberEffective.code };
            this.$http.postJson(this.api.auditAreaApply, this.rowData).then(res => {
              this.$message({
                type: 'success',
