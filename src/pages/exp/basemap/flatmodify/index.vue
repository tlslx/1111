<template>
  <el-page ref="flatmanage">
      ...
      <kindo-box :title="source.language.audit" icon="fa-search">
        <!-- <el-form :model="forms" :rules="formRules" ref="forms" label-position="right" label-width="100px" inline @submit.native.prevent @keyup.enter.native="onQuery">
          <el-form-item :label="source.language.graphName" prop="graphName">
            <el-input v-if="forms.graphId===''" @blur="(ev)=>ev.target.value = ev.target.value.trim()" v-model="forms.graphName" class="inputclass"></el-input>
          ...
          <el-form-item>
            <el-button v-if="forms.graphId===''" type="primary" @click="onQuery">{{source.language.auto}}</el-button>
    ...
    <template slot="body">
      ...
      <el-page-data>
        <!--平面图-->
        <mapSvg ref="mapSvg" :api="config.api" :onexport="false" :onsave="right('save')" :ondelete="right('delete')" :language="source.language" @reload="onChild" :imgurl="imgurl" :costom="true" title="平面图绘制" />
      </el-page-data>
    </template>
  </el-page>
</template>
  ...
  methods: {
    ...
    onResetQuery() {
      ...
      this.$nextTick(function () {
        ...
        this.$refs.mapSvg.action = "modify";
        // console.log(this.$route.query.graphId);
      });
    },
    getall() {
      this.source.expnum = [];
