...
export default {
  ...
  methods: {
    ...
    onSave() {
      ...
      rows.map(item => {
        workids.push(item.workflowId);
      });
      let param = {
        buttomGraphId: rows.map(item => item.buttomGraphId).toString(),
        businessKey: rows.map(item => item.workflowId).toString(),
        id: rows.map(item => item.taskId).toString(),
        variables: 
        {
          departIds: ids,
          departmentNames: names, 
          businessKeys: workids.toString(),
          nonCheckPermission: "true"
        }
      };
      this.$http.postJson(this.api.consult, param).then(res => {
