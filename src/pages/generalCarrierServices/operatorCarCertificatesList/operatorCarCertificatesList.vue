...
<template>
  <div class="container">
    <p class="top_line"><span class="title">经办人车证申请审核</span></p>
    ...
    <template>
      <el-table
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <router-link :to="{ path: '/carrierCmInfoDetail', query: { carrierId: scope.row.carrierId, enterRouter: '/operatorCarCertificatesList' }}">{{scope.row.carrierName}}</router-link>
          </template>
        </el-table-column>
        <el-table-column
          label="车证种类"
          ...
          width="100">
          <template slot-scope="scope">
            <router-link v-if="scope.row.cardType === '0'" :to="{ path: '/cantonFairCarCertificatesDetail', query: { vehicleCardId: scope.row.vehicleCardId, enterRouter: '/operatorCarCertificatesList' }}">筹展车证</router-link>
            <router-link v-if="scope.row.cardType === '1'" :to="{ path: '/cantonFairCarCertificatesDetail', query: { vehicleCardId: scope.row.vehicleCardId, enterRouter: '/operatorCarCertificatesList' }}">撤展车证</router-link>
            <router-link v-if="scope.row.cardType === '2'" :to="{ path: '/cantonFairCarCertificatesDetail', query: { vehicleCardId: scope.row.vehicleCardId, enterRouter: '/operatorCarCertificatesList' }}">叉车证</router-link>
          </template>
        </el-table-column>
        <el-table-column
          prop="status"
    ...
    methods: {
      ...
      */
      searchForumList(flag) {
        ...
        this.$http.get(this.baseApi + '/api/customservice/carrier/listAuditVehicleCardFirst', this.ruleForm).then(res => {
      ...
      */
      ...
      sendPassRequest(vehicleCardIds) {
        this.$http.post(this.baseApi + '/api/customservice/carrier/passVehicleCardFirst', { vehicleCardIds: vehicleCardIds }).then(res => {
      ...
      sendNoPassRequest() {
        this.$refs["resonForm"].validate((valid) => {
          if (valid) {
            this.$http.postJson(this.baseApi + '/api/customservice/carrier/rejectVehicleCardFirst', this.resonForm).then(res => {
              this.$message.success("操作成功!");
