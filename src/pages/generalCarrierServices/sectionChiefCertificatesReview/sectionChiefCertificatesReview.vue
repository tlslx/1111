...
<template>
  <div class="container">
    <p class="top_line"><span class="title">科长人证申请审核</span></p>
    ...
    <template>
      <el-table
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <router-link :to="{ path: '/carrierCmInfoDetail', query: { carrierId: scope.row.carrierId, enterRouter: '/sectionChiefCertificatesReview' }}">{{scope.row.carrierName}}</router-link>
          </template>
        </el-table-column>
        <el-table-column
          prop="name"
    ...
    methods: {
      ...
      */
      searchForumList(flag) {
        ...
        this.$http.get(this.baseApi + '/api/customservice/carrier/listAuditManCardSecond', this.ruleForm).then(res => {
      ...
      */
      ...
      sendPassRequest(manCardIds) {
        this.$http.post(this.baseApi + '/api/customservice/carrier/passManCardSecond', { manCardIds: manCardIds }).then(res => {
      ...
      sendNoPassRequest() {
        this.$refs["resonForm"].validate((valid) => {
          if (valid) {
            this.$http.postJson(this.baseApi + '/api/customservice/carrier/rejectManCardSecond', this.resonForm).then(res => {
              this.$message.success("操作成功!");
