...
export default {
  ...
  getProgressTime(params) {
    return new Promise((resolve, reject) => {
      $req.get(IMPORT_URL + '/api/ot/progress/getProgressTime?exhibitionNum=' + params.exhibitionNum + '&subKey=' + params.subKey).then(res => {
        resolve(res);
      }).catch(e => {
        reject(e);
      });
  ...
  // 获取特例限制
  getRuleInfo(params) {
    let queryParams = 'exhibitionNum=' + params.exhibitionNum + '&subKey=' + params.subKey + '&orgId=' + params.orgId;
